...
#ifndef TENACITAS_LIB_GENERIC_FORMAT_NUMBER_H
#define TENACITAS_LIB_GENERIC_FORMAT_NUMBER_H
...
#include <tenacitas.lib/generic/internal/max_str_length.h>
...
requires(std::integral<t_num_type> || std::floating_point<t_num_type>)
  ...
  _stream << (p_align == align::right ? std::right : std::left) << std::fixed
          << std::setfill(p_fill)
          << std::setw(internal::max_str_length<t_num_type>())
