...
//// message to be sent
// struct temperature {
//    explicit temperature(uint16_t p_idx = 0, float p_value = 0.0)
//        : idx(p_idx)
//        , value(p_value) {}

//    friend std::ostream &operator<<(std::ostream &p_out,
//                                    const temperature &p_temperature) {
//        p_out << '[' << p_temperature.idx << ',' << p_temperature.value <<
//        ']'; return p_out;
//    }
...
//    //    ~temperature_sensor() { m_temperature_generator.stop(); }

// private:
//    // type for the asynchronous loop that will call the 'generator' method
//    typedef async::sleeping_loop temperature_generator;

// private:
//    // function to be executed inside the sleeping loop, that will generate
//    the
//    // temperatures
//    void generator() {
//        if (m_counter >= m_max) {
...
//        }
//        m_temperature += 0.2;
//        async::dispatch(temperature {++m_counter, m_temperature});
//    }
...
//    // adding the function that will handle temperatures to an unamed handling
//    // of temperature handlers; 2s is the time _temperature_handler has to
//    // execute
...
int main() {
    ...
    auto _temperature_handler = [&_counter_received,
                                 &_cond](type::ptr<bool>,
                                         temperature &&p_temperature) -> void {
    ...
    async::add_handler<temperature>(_temperature_handler, 2s);
    ...
    _cond.wait(_lock, [&]() -> bool { return (_counter_received >= _max); });
}
