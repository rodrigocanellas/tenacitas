...
int main() {
    ...
    auto _h1 = [&_handler_1](type::ptr<bool> p_bool,
                             temperature &&p_temperature) -> void {
        _handler_1(p_bool, std::move(p_temperature));
    };

    // lambda 2, so that _handler_2 can be used later
    auto _h2 = [&_handler_2](type::ptr<bool> p_bool,
                             temperature &&p_temperature) -> void {
        _handler_2(p_bool, std::move(p_temperature));
    };

    // adds a handler to a handling, and save the id of this handling
    async::handling_id _handling_id = async::add_handler<temperature>(_h1, 2s);

    // adds another handler to _handlers_id
    async::add_handler<temperature>(_handling_id, _h2);
