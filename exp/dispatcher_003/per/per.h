...
struct saver {
  ...
  std::optional<dom::test_id>
  operator()(dom::summary &&p_summary, dom::generators &&p_generators,
    ...
    try {
      ...
      return std::optional<dom::test_id>();
    }
    return std::optional<dom::test_id>{m_test_id};
  }
...
private:
  ...
  void save_summary(const dom::summary &p_summary) {
  ...
  void save_generators(const dom::generators &p_generators) {
    for (const dom::generator &_generator : p_generators) {
      std::stringstream _stream;
  ...
  void save_subscribers(tenacitas::lib::asy::queue_id p_publishing_id,
                        const dom::subscribers_results &p_subscribers) {
    for (const dom::subscribers_results::value_type &_value : p_subscribers) {
      std::stringstream _stream;
  ...
  void save_publishings(const dom::publishings_results &p_publishings) {
    for (const dom::publishings_results::value_type &_value : p_publishings) {
      tenacitas::lib::asy::queue_id _publishing_id = _value.first;
      std::stringstream _stream;
      ...
      if ((sqlite3_exec(m_db, _stream.str().c_str(), nullptr, nullptr, &_err) ==
          ...
          (_err == nullptr)) {
        TNCT_LOG_TST('\'', _stream.str(), "' OK");
        const dom::subscribers_results &_subscribers =
