...
struct results {
  void increment(event_id p_event_id, src::async::handling_id p_handling_id,
  ...
  std::string report() const {
    ...
    while (_ite != _end) {
      ...
      while ((_ite != _end) && (_event_prior == get_event_id(_ite))) {
        src::async::handling_id _handling_prior{get_handling_id(_ite)};
...
private:
  using key = std::tuple<event_id, src::async::handling_id, handler_type_id,
                         handler_id>;
  using value = size_t;
  ...
  using handlings_totals = std::map<src::async::handling_id, handling_total>;
...
private:
  void increment(events_totals &p_events_totals, event_id p_event_id,
                 const src::async::handling_id &p_handling_id,
                 handler_type_id p_handler_type_id, handler_id p_handler_id,
  ...
  src::async::handling_id get_handling_id(map::const_iterator p_ite) const {
  ...
  src::async::handling_id
  get_handling_id(handlings_totals::const_iterator p_ite) const {
    return p_ite->first;
  }
