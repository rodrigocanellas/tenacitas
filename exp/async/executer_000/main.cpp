...
#include "tenacitas/src/async/exec_sync.h"
...
void executer_000() {
  ...
  std::cout << (src::async::exec_sync(200ms, _function, std::move(_i))
                    ? "true"
                    : "false")
            << '\n';
}
...
void executer_002() {
  ...
  std::cout << (src::async::exec_sync(200ms, _function) ? "true" : "false")
            << '\n';
}
...
void executer_004() {
  ...
  std::cout << (src::async::exec_sync(200ms, _function, 4, std::move(_str),
                                      "goodbye")
                    ? "true"
                    : "false")
            << '\n';
...
void executer_006() {
  ...
  std::optional<int16_t> _maybe =
      src::async::exec_sync(200ms, _function, std::move(_i));
...
void executer_008() {
  ...
  std::optional<int16_t> _maybe = src::async::exec_sync(200ms, _function);
  if (!_maybe) {
...
void executer_010() {
  ...
  std::optional<float> _maybe =
      src::async::exec_sync(200ms, _function, 4, -2.5);
  if (!_maybe) {
