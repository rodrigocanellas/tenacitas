...
void executer_000() {
    ...
    std::cout << (async::execute(200ms, _function, std::move(_i)) ? "true"
                                                                  : "false")
              << '\n';
}
...
void executer_004() {
    ...
    std::cout << (async::execute(200ms, _function, 4, std::move(_str),
                                 "goodbye")
                      ? "true"
                      : "false")
              << '\n';
...
void executer_009() {
    ...
    std::optional<int16_t> _maybe = async::execute(200ms, _function);
    if (_maybe) {
        std::cout << "function not timedout, as expected, and returned "
                  << *_maybe << '\n';
        return;
    }
}
...
void executer_010() {
    ...
    if (_value != static_cast<float>(-10)) {
        std::cout << "value should be " << static_cast<float>(-10)
                  << ", but it is " << _value << '\n';
    }
}
