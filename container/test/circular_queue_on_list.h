...
#ifndef TENACITAS_LIB_CONTAINER_TEST_CIRCULAR_QUEUE_H
#define TENACITAS_LIB_CONTAINER_TEST_CIRCULAR_QUEUE_H
...
public:
  using queue = container::circular_queue_on_list<log::cerr, int16_t>;

protected:
  bool check(const queue &p_queue, size_t p_occupied, size_t p_capacity,
             log::cerr &p_log) {
    if (p_queue.capacity() != p_capacity) {
      p_log.err(generic::fmt("capacity should be ", p_capacity, " but it is ",
                             p_queue.capacity()));
      return false;
    }

    if (p_queue.occupied() != p_occupied) {
      p_log.err(generic::fmt("there should be ", p_occupied,
  ...
  bool get(queue &p_queue, queue::data p_value, size_t p_occupied,
    ...
    queue::data _data;
    {
      ...
      if (!_maybe.has_value()) {
        p_log.err(
            generic::fmt("no data read, when it should have been ", p_value));
        return false;
      }
    ...
    if (_data != p_value) {
      p_log.err(
          generic::fmt("data should be ", p_value, ", but it is ", _data));
      return false;
    }

    p_log.tst(generic::fmt("getting ", _data));
  ...
  bool add(queue &p_queue, queue::data p_value, size_t p_occupied,
           size_t p_capacity, log::cerr &p_log) {
    p_log.tst(generic::fmt("adding ", p_value));
    ...
    //   p_log.err(generic::fmt("It should be", (_full ? "" : " not"),
...
struct circular_queue_009 : public circular_queue_test_base {
  ...
  bool operator()(const generic::program_options &) {
    // using namespace tenacitas::test::container::circular_queue;
    log::cerr _log;

    try {

      container::circular_queue_on_list<log::cerr, std::string> queue("q009", _log,
                                                              100000);
      ...
      while (!queue.empty()) {
        ...
        if (!maybe) {
          _log.err(generic::fmt("error getting data # ", j));
          return false;
        }
        _log.tst(generic::fmt("capacity = ", queue.capacity(), ", occupied = ",
...
struct circular_queue_012 : public circular_queue_test_base {
  static std::string desc() {
    return "Tests if 'container::circular_queue' conforms to 'traits::queue";
  }
