#ifndef TNCT_MATH_BUS_MUL_H
#define TNCT_MATH_BUS_MUL_H
...
[[nodiscard]] constexpr std::optional<t_result> mul(t_first  p_first,
                                                    t_second p_second)
{
  if constexpr (std::is_signed_v<t_first> == std::is_signed_v<t_second>)
  {
    return internal::mul_same_operands<t_result>(p_first, p_second);
  }
  else if constexpr (std::is_signed_v<t_result>)
  {
    return internal::mul_different_operands_signed_result<t_result>(p_first,
                                                                    p_second);
  }
  else
  {
    return internal::mul_different_operands_unsigned_result<t_result>(p_first,
                                                                      p_second);
  }
}
