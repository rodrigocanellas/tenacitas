...
#ifndef TNCT_TRAITS_TEST_TUPLE_CONTAINS_TUPLE_TEST_H
#define TNCT_TRAITS_TEST_TUPLE_CONTAINS_TUPLE_TEST_H
...
'traits::tuple_contains_tuple<std::tuple<int, float>, std::tuple<int, char> >'
evaluated to false tuple_contains_tuple.h:32:9: because '[]<std::size_t
...t_idx>(std::index_sequence<t_idx...>) { return
((is_type_in_tuple<std::tuple<int, float>, std::tuple_element_t<t_idx,
tuple<int, char> > >) && ...);
}(std::make_index_sequence<std::tuple_size_v<tuple<int, char> > >())' evaluated
to false
     */
    // func<tuple_1, tuple_2>();
