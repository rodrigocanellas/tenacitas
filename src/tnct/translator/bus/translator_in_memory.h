...
#ifndef TNCT_TRANSLATOR_TRANSLATOR_IN_MEMORY_H
#define TNCT_TRANSLATOR_TRANSLATOR_IN_MEMORY_H

#include <cstdint>
#include <string>

#include "tnct/translator/dat/dictionary.h"
...
struct translator_in_memory
{
  ...
  translator_in_memory(dat::dictionary<t_index> &&p_dictionary)
  ...
  uint32_t inline size() const
  {
    return m_dictionary.size();
  }

  inline std::string get(t_index p_word_id)
  {
  ...
  void inline add(dat::dictionary<t_index> &&p_dictionary)
  {
...
private:
  dat::dictionary<t_index> m_dictionary;
};
