...
#ifndef TNCT_TESTER_TEST_H
#define TNCT_TESTER_TEST_H
...
/// \brief Runs a test
///
/// \param test is an instance of tnct::test defined
/// below
///
/// \param test is the name of a class that implements
///
/// \code
/// bool operator()(const tnct::program::bus::options &)
///
...
struct test
{
  ...
  test(int argc, char **argv,
       std::initializer_list<program::bus::options::name> &&p_mandatory =
           {}) noexcept
      : m_argc(argc), m_argv(argv)
  {
    ...
    try
    {
      ...
      {
        std::optional<std::list<program::bus::options::value>> _maybe =
            m_options.get_set_param("exec");
        if (_maybe)
        {
          m_execute_tests = true;
          std::list<program::bus::options::value> _tests_to_exec =
              std::move(*_maybe);
