...
using matrix = container::dat::matrix<std::size_t, std::size_t>;
...
std::optional<std::size_t> get_matriz_size(logger &p_logger,
  ...
  try {
    ...
    TNCT_LOG_ERR(p_logger, format::bus::fmt(p_param, " is not a valid number"));
  }
  return std::nullopt;
}
...
using dispatcher =
    async::bus::dispatcher<logger, tnct::async::exp::evt::matrix_summed>;
...
private:
  ...
  using internal_dispatcher =
      async::bus::dispatcher<logger, sum_line, line_summed>;
  ...
  struct matrix_summer {
    ...
    void operator()(line_summed &&p_line_summed) {
      ...
      if (all_rows_summed()) {
        auto _result{
            m_dispatcher.publish<tnct::async::exp::evt::matrix_summed>(m_sum)};
        if (_result != async::dat::result::OK) {
          TNCT_LOG_ERR(
              m_logger,
              format::bus::fmt("error publishing 'matrix_summed': ", _result));
        }
      }
    }
...
void async_sum_matrix_wrapper(matrix &p_matrix, logger &p_logger) {
  ...
  using matrix_summed_queue =
      container::dat::circular_queue<logger,
                                     tnct::async::exp::evt::matrix_summed>;
  ...
  auto _end_of_pgm = [&](tnct::async::exp::evt::matrix_summed &&p_event) {
    _sum = p_event.m_sum;
    _cond.notify_one();
  };

  _dispatcher.add_handling<tnct::async::exp::evt::matrix_summed>(
