...
struct pgm
{
  ...
  int operator()(int argc, char **argv)
  {
    ...
    try
    {
      ...
      async::exp::configuration _configuration(_logger, argv[1]);
...
private:
  ...
  struct event_handled_handler
  {
    ...
    event_handled_handler(async::exp::logger &p_logger, size_t &p_total_handled,
                          async::exp::results     &p_results,
  ...
  private:
    async::exp::logger      &m_logger;
    size_t                  &m_total_handled;
    async::exp::results     &m_results;
    size_t                   m_percentage_to_display;
    size_t                   m_total_to_be_handled;
    std::condition_variable &m_cond_all_handled;
  };

  void define_handlings(dispatcher &p_dispatcher, async::exp::logger &p_logger,
