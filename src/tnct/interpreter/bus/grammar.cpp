...
//     /// productions emerge;
//     /// still, this is production that ultimately must be recognized by a
//     parser add_production ( production_id ( "!root-prod!" ) ) ;
...
// //   memory::ptr<basic::exception> e;

// //   if  ( productions_.size ( )  < 2 )  {
// //     e = new basic::exception  ( grammar_class_name, "add_grammar",
// //                               error_import_prod_with_empty_grammar ) ;
// //   }

// //   if  (  ( !e )  &&  ( !already_imported ( g->get_id ( ) ) )  )  {

// //     // importing productions
// //     productions::const_iterator ite_prod = ++ ( g->productions_.begin ( )
// ) ;
// //     memory::ptr<production> prod;
// //     for  ( ; ite_prod != g->productions_.end ( ) ; ++ite_prod )  {
// //       prod = find_production ( ( *ite_prod ) ->get_id ( ) ) ;
// //       if  ( prod )  {
// //         e = new basic::exception ( grammar_class_name, "add_grammar",
// //                                  error_import_prod_collision_name ) ;
// //       }
// //       else {
// //         productions_.push_back  ( *ite_prod ) ;
// //       }
// //     }

// //     if  ( !e )  {
// //       // importing symbols
// //       symbols_ids::const_iterator ite_symb_id = g->reserved_symbols_.begin
// ( ) ;
// //       for  ( ; ite_symb_id != g->reserved_symbols_.end ( ) ; ++ite_symb_id
// )  {
// //         memory::ptr<const symbol_id> symb = find_symbol_id ( * (
// *ite_symb_id ) ) ;
// //         if  ( !symb )
// //           reserved_symbols_.push_back ( *ite_symb_id ) ;
// //       }

// //       // importing routines
// //       routines::const_iterator ite_routine = g->routines_.begin ( ) ;
// //       for  ( ; ite_routine != g->routines_.end ( ) ; ++ite_routine )  {
// //         memory::ptr<const routine> rout = find_routine ( ( *ite_routine )
// ->get_id ( ) ) ;
// //         if  ( !rout )
// //           routines_.push_back ( *ite_routine ) ;

// //         // If a routine already exists in the current grammar with the
// same
// //         // name as in the imported grammar, I understand that it is a
// //         // routine being reused.

// //         //       else {
// //         //         throw basic::exception ( grammar_class_name,
// "add_grammar",
// //         //                                error_import_rout_collision_name
// ) ;
// //         //       }
// //       }

// //       // importing types
// //       types::const_iterator ite_type = g->types_.begin ( ) ;
// //       for  ( ; ite_type != g->types_.end ( ) ; ++ite_type )  {
// //         memory::ptr<const type> t = find_type ( ( *ite_type ) ->get_id ( )
// ) ;
// //         if  ( !t )
// //           types_.push_back ( *ite_type ) ;
// //       }
...
// // const tenacitas::interpreter::recognizer<
// //     tenacitas::identifier::id<
// //         std::basic_string<char, std::char_traits<char>,
// std::allocator<char> >,
// //         tenacitas::identifier::validator<std::basic_string<char,
// std::char_traits<char>, std::allocator<char> > >,
// //         tenacitas::interpreter::symbol::value_default> >*’ to
// // ‘const tenacitas::interpreter::value_recognizer*’
...
//     if  ( l_reco ) {
//         throw  ( create_fault ( grammar, error_recognizer_exists ) ) ;
//     }
...
// void
// grammar::
// add_production  ( const production_id & p_prod_id ) throw ( number::fault ) {

//     memory::ptr<production> l_prod = find_production ( p_prod_id ) ;

//     if  ( l_prod ) {
//         throw ( create_fault ( grammar, error_production_exists ) ) ;
//     }
...
// void
// grammar::
// add_routine ( memory::ptr<const routine> p_rout ) throw  ( number::fault ) {

//     memory::ptr<const routine> l_rout = find_routine ( p_rout->get_id ( ) ) ;

//     if  ( l_rout ) {
//         throw  ( create_fault ( grammar, error_routine_exists ) ) ;
//     }
...
//     if  ( !l_prod ) {
//         throw  ( create_fault ( grammar, error_non_existing_production ) ) ;
//     }

//     memory::ptr<production::item> l_item = l_prod->find ( p_item_id ) ;

//     if  ( l_item )  {
//         throw  ( create_fault ( grammar, error_item_exists ) ) ;
//     }
...
// 		 const production_id & prod_id ) throw ( number::fault )  {
...
//     if  ( !l_item ) {
//         throw  ( create_fault  ( grammar, error_non_existing_item ) ) ;
//     }

//     if  ( l_item->points_to_type ( ) ) {
//         throw  ( create_fault  ( grammar, error_item_refs_type ) ) ;
//     }

//     if  ( l_item->points_to_value ( ) ) {
//         throw  ( create_fault  ( grammar, error_item_refs_value ) ) ;
//     }

//     if  ( l_item->points_to_production ( ) ) {
//         throw  ( create_fault  ( grammar, error_item_refs_production ) ) ;
//     }
...
//     ) ; if  ( !l_symbol_type ) {
//         throw  ( create_fault ( grammar, error_non_existing_recognizer ) ) ;
//     }
...
// // void
// // grammar::
// // set_routine ( const production_id &p_prod_id,
// //               const item_id & p_item_id,
// //               const routine_id & p_rout_id )  throw  ( number::fault )  {
...
// //     memory::ptr<const routine> l_rout = find_routine ( p_rout_id ) ;
// //     if  ( !l_rout ) {
// //         throw  ( create_fault ( grammar, error_non_existing_routine ) ) ;
// //     }
...
// //     if ( l_item->get_routine ( ) != 0 ) {
// //         throw ( create_fault ( grammar, error_item_refs_routine ) ) ;
// //     }

// //     l_item->set_routine ( l_rout ) ;
// // }

// // ---------------
// // void
// // grammar::
// // set_routine ( const routine_id & p_rout_id )throw  ( number::fault )  {
...
// //     memory::ptr<production> l_prod = get_first_production ( ) ;
// //     if  ( !l_prod ) {
// //         throw  ( create_fault  ( grammar, error_non_existing_production )
// ) ;
// //     }

// //     memory::ptr<production::item> l_item = l_prod->get_first ( )  ;
// //     if  ( !l_item ) {
// //         throw  ( create_fault ( grammar, error_non_existing_item ) ) ;
// //     }
...
//     memory::ptr<const production::item> l_suc = l_prod->find ( p_suc_id ) ;
//     if  ( !l_suc )  {
//         throw  ( create_fault  ( grammar, error_non_existing_item ) ) ;
//     }

//     if  ( !l_item->get_suc( )->is_last ( ) ) {
//         throw  ( create_fault  ( grammar, error_item_has_successor ) ) ;
//     }
...
//     if  ( l_item->get_alt ( ) ) {
//         throw  ( create_fault  ( grammar, error_item_has_alternative ) ) ;
//     }
...
// ---------------
// static const i16t mark_w = 2;
// static const i16t owner_w = 25;
// static const i16t item_w  = 7;
// static const i16t type_w = 25;
// static const i16t value_w = 25;
// static const i16t prod_w = owner_w;
// static const i16t routine_w = 15;
// static const i16t suc_w = item_w;
// static const i16t alt_w = item_w;
...
//         /// the currenty analysed @p production was not found in this @p
//         grammar,
...
//             /// recognizer_id of the currently analysed @p type_recognizer
//             was
//             /// found, so we add it to the @p type_recognizers collection of
//             this
//             /// @p grammar
...
// //     if  ( i->get_id ( )  != id )
// //       throw  ( basic::exception ( "production", "add",
// //                               grammar::error_invalid_item_reference ) ) ;
...
//                  , m_id, points_to_type ( ) ? *get_symbol_type ( ) :
//                  symbol::type ( ) , points_to_value ( ) ? *get_symbol_value (
//                  ) : symbol::value ( ) , points_to_production ( ) ?
//                  get_production ( )->get_id ( ) : production_id ( ) ,
