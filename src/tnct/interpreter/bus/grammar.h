...
#ifndef TNCT_INTERPRETER_GRAMMAR_H
#define TNCT_INTERPRETER_GRAMMAR_H
...
#include <tenacitas.identifier/identifier.h>
...
/** @brief Functor that defines a default value for a
    tenacitas::interpreter::grammar_id */
class grammar_default {
public:
  typedef string value_type;

  static string value() { return "!!no-grammar-id!!"; }
};

/** @brief Grammar identifier */
typedef identifier::id<string, identifier::validator<string>, grammar_default>
    grammar_id;

/** @brief Item identifier */
typedef identifier::id<i16t> item_id;

/** @brief The @p grammar class represents a computer programming
    grammar,  usually to define a programming language. However, it
    can be used to define any formal combination of symbols to be
    used in many situatons, i. e., any syntax.

    Grammars are usually defined in Backus–Naur form (BNF). You can
    read more about BNF in http:
    //en.wikipedia.org/wiki/Backus-Naur_form, among many other, and
    as an example, you can read an the BNF for the Pascal language in
    ...
    comparison of an input text to the grammar, to check if the text
    is correct according to the  grammar, normally called "parsing
    the text" or "syntax analisys",  is done by class
    ...
    The <tt> unary-expression </tt> @p production has an @p item, say
    0, which references the <tt> unary-op </tt> @p production; item 0
    is followed by another @p item, say 1, which references the <tt>
    unary-expression </tt> @p production; @p item 0 has an
    alternative item, say 2, which references <tt> primary-expression
    </tt>
    @p production.

    - a tenacitas::interpreter::symbol::type, which means that it is
    possible to recognize a tenacitas::interpreter::symbol directly
    from the input text, such as, in Pascal: <tt> primary-expression:
    unsigned-integer word
    </tt>

    The @p production <tt> primary-expression </tt> has an @p item,
    say 0, which references the @p type <tt> unsigned-integer </tt>,
    which is a combination of digits from 0 to 9, and no '-' sign. @p
    item 0 has an alternative @p item, say 1, which references the @p
    symbol::type @p word, which is a combination of letters. Both
    <tt> unsigned-integer </tt> and @p word are defined by a format
    which can be recognized directly from the input text.

    - a tenacitas::interpreter::symbol::value, a defined combination
    of characters, like '==' or 'const'.

    When a tenacitas::interpreter::item references a
    tenacitas::interpreter::production, the
    tenacitas::interpreter::parser object recursively starts to
    analyse this production.

    A tenacitas::interpreter::item object can also reference a
    tenacitas::interpreter::routine object, which is a function that
    the tenacitas::interpreter::parser object will execute when the
    ...
    This means that one can implement a @ grammar for an @p
    expression, and have, for different situations, different
    tenacitas::interpreter::routine objects associated.

    So, if in one situation all we want is to print the @p
    expression, we can write tenacitas::interpreter::routine objects
    to simply print the elements of the expression.
    ...
    Finally, we can build new tenacitas::interpreter::grammar based
    on existing ones. Suppose we create a
    tenacitas::interpreter::expression class, sub-class of @p
    grammar. We can later create a  tenacitas::interpreter::pascal,
    also sub-class of  @p grammar, that incorporates the
...
public:
  ...
  memory::ptr<const type_recognizers> get_type_recognizers() const;

  /** @brief Retrieves the collection of
      tenacitas::interpreter::tokens objects */
  memory::ptr<const tokens> get_tokens() const;

  /** @brief Retrieves the first production of the @p grammar */
  memory::ptr<production> get_first_production() const;
  ...
  static const number::fault::code error_production_colision;

  static const number::fault::code error_routine_colision;

  static const number::fault::code error_production_exists;

  static const number::fault::code error_routine_exists;

  static const number::fault::code error_item_exists;

  static const number::fault::code error_non_existing_production;

  static const number::fault::code error_non_existing_item;

  static const number::fault::code error_item_refs_type;

  static const number::fault::code error_item_refs_value;

  static const number::fault::code error_non_existing_recognizer;

  //             static const number::fault::code error_non_existing_routine;

  static const number::fault::code error_item_refs_production;

  static const number::fault::code error_item_has_successor;

  static const number::fault::code error_item_has_alternative;

  //             static const number::fault::code error_item_refs_routine ;

  static const number::fault::code error_recognizer_exists;
...
protected:
  ...
  /** @brief The productions of the grammar type
      ...
      That is ok to be a simple <tt> std::list </tt> because all
      the manipulation of the collection will be internal, i.e.,
      the programmer user of the @p interpreter project will not
      use it */
  typedef list<memory::ptr<production>> productions;
  ...
  /** @brief Incorporates an existing grammar into this grammar
      ...
      error_production_colision </tt> if @p p_grammar has a
      tenacitas::interpreter::production with the same
      tenacitas::interpreter::production_id that an existing @p
      production in this @p grammar.

      @throw tenacitas::number::fault with <tt> get_code ( ) ==
      error_routine_colision </tt> if @p p_grammar has a
      tenacitas::interpreter::routine with the same
      tenacitas::interpreter::routine_id that a existing @p
      routine in this @p grammar. */
  template <typename t_grammar> void add_grammar() throw(number::fault);
  ...
  /** @brief Adds a tenacitas::interpreter::type_recognizer to
      ...
      @throw tenacitas::number::fault with <tt> get_code ( ) ==
      error_recognizer_exists </tt> if there already exists a
      tenacitas::interpreter::recognizer_byt_type which <tt> get_id
      ( ) == p_recognizer->get_id ( ) </tt> */
  void add_type_recognizer(
      memory::ptr<const type_recognizer> p_recognizer) throw(number::fault);
  ...
  /** @brief Adds a group of values to be recognized
      ...
      shall recognize from the input text, like like "= == ;"
      ...
      @throw tenacitas::number::fault with <tt> get_code ( ) ==
      error_recognizer_exists </tt> if there already exists a
      tenacitas::interpreter::recognizer_byt_type which that
      recognizes one of the strings coded in @p p_values */
  void add_token(const string &p_values,
                 const symbol::type &p_type) throw(number::fault);
  ...
  /** @brief Adds a @p tenacitas::interpreter::production to the @p
      ...
      @throw tenacitas::number::fault with <tt> get_code ( ) ==
      error_production_exists </tt> if there already exists a @p
      production wich <tt> get_id ( ) == p_prod_id </tt> */
  void add_production(const production_id &p_prod_id) throw(number::fault);

  /** @brief Adds a tenacitas::interpreter::routine to the @p
  grammar
      ...
      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_routine_exists </tt> if there already exists a @p
      routine which <tt> get_id ( ) == p_rout->get_id ( ) </tt> */
  void add_routine(memory::ptr<const routine> p_rout) throw(number::fault);
  ...
  /** Adds a tenacitas::interpreter::item to a previously added
      ...
      @throw number::fault with <tt> get_code ( ) ==
      error_item_exists </tt> if there already exists a @p item in
      @p p_prod which <tt> get_id ( ) == p_item_id </tt>  */
  void add_item(const production_id &p_prod_id,
                const item_id &p_item_id) throw(number::fault);
  ...
  /** @brief Defines that a tenacitas::interpreter::item in a
      ...
      @param [in] p_item_id is the identifier of the @p item that
      will reference the  @p production which identifier is @p
      p_prod_id

      @param [in] p_prod_id is the identifier of the @p production
      that will be referenced by @p item @p p_item_id, which is
      held by the @p production @p p_prod_id_owner

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_production </tt> if there is no @p
      production which <tt> get_id ( ) == p_prod_id_owner </tt>

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_item </tt> if there is no @p item in the
      @p production @p p_prod_owner which <tt> get_id ( ) ==
      p_item_id
      </tt>

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_production </tt> if there is not @p
      production which <tt> get_id ( ) == p_prod_id </tt>

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_item_refs_type </tt> if @p p_item_id already references
      a tenacitas::interpreter::symbol::type object

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_item_refs_value </tt> if @p p_item_id already
      references a tenacitas::interpreter::symbol::value object

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_item_refs_production </tt> if @p p_item_id already
      references a tenacitas::interpreter::production object */
  void set_production(const production_id &p_prod_id_owner,
                      const item_id &p_item_id,
                      const production_id &p_prod_id) throw(number::fault);
  ...
  /** Defines that a tenacitas::interpreter::item in a
      ...
      @param [in] p_item_id is the identifier of the @p item that
      will reference the @p type which identifier is @p
      p_symbol_type

      @param [in] p_symbol_type is the identifier of the @p
      symbol::type that will be referenced by the @p item @p
      p_item, which is held by the @p production @p p_prod_id
      ...
      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_item </tt> if there is no @p item in the
      @p production @p p_prod which <tt> get_id ( ) == p_item_id
      </tt>.

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_recognizer </tt> if there is no registered
      @p recognizer that recognizes @p p_symbol_type
      ...
      error_item_refs_production </tt> if @p p_item_id already
      references a tenacitas::interpreter::production object */
  void set_type(const production_id &p_prod_id, const item_id &p_item_id,
                const symbol::type &p_symbol_type) throw(number::fault);
  ...
  /** Defines that a tenacitas::interpreter::item in a
      ...
      symbol::value that will be referenced by the @p item @p
      p_item, which is held by the @p production @p p_prod_id
      ...
      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_recognizer </tt> if there is no registered
      @p recognizer that recognizes @p p_symbol_value
      ...
      error_item_refs_production </tt> if @p p_item_id already
      references a tenacitas::interpreter::production object */
  void set_value(const production_id &p_prod_id, const item_id &p_item_id,
                 const symbol::value &p_symbol_value) throw(number::fault);

  /** @brief Makes all the
      tenacitas::interpreter::grammar::production::item that
      references a tenacitas::interpreter::grammar::production
      which <tt> get_id ( ) == p_routine->get_prod_id ( ) </tt> to
      reference
      @p p_routine */
  void add_production_routine(memory::ptr<const production_routine> p_routine);

  /** @brief Makes all the
      tenacitas::interpreter::grammar::production::item that
      references a tenacitas::interpreter::grammar::symbol::type
      which <tt> get_symbol_type ( ) == p_routine->get_type ( )
      </tt> to reference @p p_routine  */
  void add_type_routine(memory::ptr<const symbol_type_routine> p_routine);

  /** @brief  Makes all the
      tenacitas::interpreter::grammar::production::item that
      references a tenacitas::interpreter::grammar::symbol::symbol
      which <tt> get_symbol_value ( ) == p_routine->get_value ( )
      </tt> to reference @p p_routine  */
  void add_value_routine(memory::ptr<const symbol_value_routine> p_routine);
  ...
  /** @brief Defines the successor tenacitas::interpreter::item
      ...
      @param [in] p_item_id is the identifier of the @p item that
      will have its successor @p item defined

      @param [in] p_suc_id is the identifier of the @p item that
      will be successor of @p p_item_id
      ...
      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_item </tt> if there is no @p item in the
      @p production @p p_prod which <tt> get_id ( ) == p_suc_id
      </tt>

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_item_has_successor </tt> if @p p_item_id already has as
      successor tenacitas::interpreter::item defined */
  void set_suc(const production_id &p_prod_id, const item_id &p_item_id,
               const item_id &p_suc_id) throw(number::fault);
  ...
  /** @brief Defines the alternative tenacitas::interpreter::item
      ...
      @param [in] p_item_id is the identifier of the @p item that
      will have its alternative @p item defined

      @param [in] p_alt_id is the identifier of the @p item that
      will be alternative to @p p_item_id
      ...
      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_non_existing_item </tt> if there is no @p item in the
      @p production @p p_prod which <tt> get_id ( ) == p_alt_id
      </tt>

      @throw tenacitas::number::fault with <tt> get_code( ) ==
      error_item_has_alternative </tt> if @p p_item_id already has
      as alternative successor tenacitas::interpreter::item defined
      */
  void set_alt(const production_id &p_prod_id, const item_id &p_item_id,
               const item_id &p_alt_id) throw(number::fault);

  /** Defines that the altenative tenacitas::interpreter::item
  accepts
      ...
      @param [in] p_item_id is the identifier of the @p item that
      will accept any symbol as an alternative to the one it holds
  ...
  void accepts_anything(const production_id &p_prod,
                        const item_id &p_item) throw(number::fault);
...
private:
  ...
  /** @brief Retrieves a reference to a
      ...
      @param [in] p_rout_id is the
      tenacitas::interpreter::routine_id of the
      tenacitas::interpreter::routine one wants to retrieve

      @return a reference to the searched
      tenacitas::interpreter::routine
      @return a null reference, otherwise */
  memory::ptr<const routine> find_routine(const routine_id &p_rout_id) const;
  ...
  /** @brief Retrieves a reference to a
      ...
      tenacitas::interpreter::production
      @return A null reference, otherwise  */
  memory::ptr<production> find_production(const production_id &p_prod_id) const;
  ...
  bool already_imported(memory::ptr<const grammar> p_grammar) const;

  /** @brief Tries to import the @p production objects from another
  @p
      grammar
      ...
      i.e., if there was no @p production object in @p p_grammar
      where the @p production_id is equal to any existing @p
      production in this @p grammar

      @return @false, otherwise */
  bool import_productions(const grammar &p_grammar);

  /** @brief Tries to import the @p type_recognizers from another
  @p
      grammar
  ...
  /** @brief Tries to import the @p production_routine objects, all
  the
      @p value_routine objects, and all the @p type_routine objects
      from another @p grammar. If any of the @p routine from @p
      p_grammar has the same @p routine_id if the @p routine
      objects that exist in this grammar, the import fails
  ...
  /** @brief Imports all @p prodution_routine object from a @p
  grammar
      into this @p grammar
      @todo implement it */
  ...
  memory::ptr<type_recognizers> m_type_recognizers;

  /** @brief Values recognizeds directly from the input text */
  memory::ptr<tokens> m_tokens;
...
/** @brief   A @p production is a group of
tenacitas::interpreter::item,
    that defines sequences of valid symbols in a grammar

    Please refer to the tenacitas::interpreter::grammar for a
...
public:
  ...
  memory::ptr<item> get_first() const;
...
private:
  ...
  /** @brief Collection of tenacitas::interpreter::item objects,
  that
      form a @p production */
  typedef list<memory::ptr<item>> itens;
  ...
  /** @brief Given a tenacitas::interpreter::item_id, tries to
      ...
      of the tenacitas::interpreter::item which one wants to
      retrieve a reference to

      @return a valid reference to a tenacitas::interpreter::item,
      if there is a tenacitas::interpreter::item which
      tenacitas::interpreter::item_id is @p p_item_id

      @return a null reference otherwise; */
  memory::ptr<item> find(const item_id &p_item_id) const;

  /** @brief Adds an tenacitas::interpreter::item to a @p
  production,
      returning a reference to it.

      @param [in] p_item_id is the tenacitas::interpreter::item_id
      of the new tenacitas::interpreter::item

      @return a reference to the new tenacitas::interpreter::item
      */
  memory::ptr<item> add(const item_id &p_item_id);
  ...
  /** @brief The collection of tenacitas::interpreter::item of the
  @p
      production (*/
  itens m_itens;
};
...
public:
  ...
  /** @brief Creates an item, defining its id, with no successor,
  no
      alternative item, no tenacitas::interpreter::routine, no
      tenacitas::interpreter::symbol::type, no
      tenacitas::interpreter::symbol::value, and defining this item
      as the last one in a @p production

      @param [in] p_id is the tenacitas::interpreter::item_id of
      this
      @p item

      @param [in] p_prod is the tenacitas::interpreter::production
      to which this item can belong to */
  item(const item_id &p_id, memory::ptr<const production> p_prod = 0);
  ...
  memory::ptr<const routine> get_routine() const;

  /** @return a valid reference to a
      tenacitas::interpreter::symbol::type
      referenced by this @p item, if it exists
      @return 0, if there is no
      tenacitas::interpreter::symbol::type referenced by this @p
      item */
  memory::ptr<const symbol::type> get_symbol_type() const;

  /** @return a valid reference to a
      tenacitas::interpreter::symbol::value
      referenced by this @p item, if it exists
      @return 0, if there is no
      tenacitas::interpreter::symbol::value referenced by this @p
      item */
  memory::ptr<const symbol::value> get_symbol_value() const;

  /** @return a valid reference to a
      tenacitas::interpreter::production referenced by this @p
      item, if it exists
      @return 0, if there is no tenacitas::interpreter::production
      referenced by this @p item */
  memory::ptr<const production> get_production() const;

  /** @return the tenacitas::interpreter::production that owns this
  @p
      item */
  memory::ptr<const production> get_production_owner() const;

  /** @return a valid reference to a
      tenacitas::interpreter::item, which is the successor @p item
      of this @p item, if it exists
      @return 0, if this @p item has no successor @p item */
  memory::ptr<const item> get_suc() const;

  /** @return a valid reference to a
      tenacitas::interpreter::item, which is the alternative @p
      item of this @p item, if it exists
      @return 0, if this @p item has no alternative @p item */
  memory::ptr<const item> get_alt() const;
...
private:
  ...
  /** @brief Defines that this item references a @p
      production_routine  */
  void set_production_routine(memory::ptr<const production_routine> p_routine);
  ...
  /** @brief Defines the tenacitas::interpreter::routine referenced
  by
      this @p item

      @param [in] p_rout is a reference to the
      tenacitas::interpreter::routine that will be referenced by
      the
      @p item  */
  void set_routine(memory::ptr<const routine> p_rout);

  /** @brief Defines the tenacitas::interpreter::symbol::type
      referenced by this @p item

      @param [in] p_symbol_type is the
      tenacitas::interpreter::symbol::type reference to be
      referenced by this @p item  */
  void set_symbol_type(memory::ptr<const symbol::type> p_symbol_type);

  /** @brief Defines the tenacitas::interpreter::symbol::value
      referenced by this @p item

      @param [in] p_symbol_value is the
      tenacitas::interpreter::symbol::value reference to be
      referenced by this @p item  */
  void set_symbol_value(memory::ptr<const symbol::value> p_symbol_value);

  /** @brief Defines the tenacitas::interpreter::production
  referenced
      by this @p item

      @param [in] p_prod is the tenacitas::interpreter::production
      reference to be referenced by this @p item  */
  void set_production(memory::ptr<const production> p_prod);

  /** @brief Defines the successor item

      @param [in] p_item is the successor @p item of the @p item */
  void set_suc(memory::ptr<const item> p_item);

  /** @brief Defines the item alternative item

      @param [in] p_item is the alternative @p item of the @p item
      */
  void set_alt(memory::ptr<const item> p_item);
  ...
  /** @brief tenacitas::interpreter::item_id of the @p item
  referenced
      by the @p item that accept any
      tenacitas::interpreter::symbol_id as an alternative  */
  // static
  static const item_id accepts_any_symbol_id;

  /** @brief The "dummy" @p item is a static refence because we
  only
      need one  per tenacitas::interpreter::production, as all the
      tenacitas::interpreter::item objects that reference the  @p
      dummy_item (as successor or alternative) will reference the
      same object */
  static const memory::ptr<const item> dummy_item;
  ...
  static const memory::ptr<item> accepts_any_symbol;
...
using namespace tnct::interpreter;

//
// ############################################################################
//                                  grammar

template <typename t_grammar> void grammar::add_grammar() throw(number::fault) {
