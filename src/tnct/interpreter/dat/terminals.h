...
public:
  ...
  using recognition = std::optional<token>;
  ...
  recognition recognize(std::string::const_iterator p_begin,
...
private:
  ...
  /// lexema
  recognition is_defined(typename lexema::const_iterator p_begin,
    ...
    return recognition{token{lexema_reference<t_lexema_size>{_ite->first},
  ...
  void import(t_iterator p_begin, t_iterator p_end) {
    ...
    std::cout << __FILE__ << ':' << __LINE__ << " terminals " << *this
              << std::endl;

    for (t_iterator _ite = p_begin; _ite != p_end; ++_ite) {
      std::cout << __FILE__ << ':' << __LINE__ << " " << _ite->first << ','
                << _ite->second << '\n';
      ...
      if (std::find_if(m_container.begin(), m_container.end(),
        ...
        std::cout << __FILE__ << ':' << __LINE__ << " back "
                  << m_container.back() << std::endl;
      } else {
        std::cout << __FILE__ << ':' << __LINE__ << " " << _ite->first << ','
                  << _ite->second << "already added" << std::endl;
      }
    }
  }
