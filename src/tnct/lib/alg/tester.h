...
#ifndef TNCT_LIB_ALG_TESTER_H
#define TNCT_LIB_ALG_TESTER_H
...
/// \brief Runs a test
///
/// \param tester is an instance of tnct::tester defined
/// below
///
/// \param test is the name of a class that implements
///
/// \code
/// bool operator()(const tnct::lib::program_options &)
///
...
template <bool use = true> struct tester {
  ...
  tester(
      int argc, char **argv,
      std::initializer_list<program_options::name> &&p_mandatory = {}) noexcept
    ...
    try {
      ...
      if (m_options.get_bool_param("exec")) {
        ...
        std::optional<std::list<program_options::value>> _maybe =
            m_options.get_set_param("exec");
        if (_maybe) {
          m_execute_tests = true;
          std::list<program_options::value> _tests_to_exec = std::move(*_maybe);
