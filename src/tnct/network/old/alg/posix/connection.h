...
#include "tnct/network/alg/posix/timeout.h"
#include "tnct/network/typ/message.h"
...
template <>
struct connection_t<typ::flow::STREAM>
{
...
private:
  ...
  typ::status timeout_read(t_time p_timeout)
  {
  ...
  template <typename t_time>
  typ::status timeout_write(t_time p_timeout)
  {
  ...
  inline bool is_socket_valid() const
  {
    return m_socket > 0;
  }

  inline void set_socket_invalid()
  {
    m_socket = -1;
  }
