#ifndef TNCT_CROSSWORDS_BUS_INTERNAL_ORGANIZER_H
#define TNCT_CROSSWORDS_BUS_INTERNAL_ORGANIZER_H
...
private:
  ...
  bool all_words_fit(const dat::grid &p_grid)
  {
    ...
    const dat::index _row_size{p_grid.get_num_rows()};

    const dat::index _col_size{p_grid.get_num_cols()};
    ...
    for (dat::grid::const_layout_ite _layout = p_grid.begin(); _layout != _end;
         ++_layout)
    {
      const dat::index _word_size{
          static_cast<dat::index>(_layout->get_word().size())};
  ...
  private:
    bool horizontal(bool &p_stop, dat::grid &p_grid)
    {
      ...
      for (dat::index _row = 0; !p_stop && (_row < _num_rows) && !_set; ++_row)
      {
                                      ...
                                      const dat::word &p_to_position)
  {
    ...
    const dat::index _size1{static_cast<dat::index>(p_positioned.size())};

    const dat::index _size2{static_cast<dat::index>(p_to_position.size())};
                             ...
                             dat::grid::layout_ite       p_to_position)
  {
    ...
    const dat::index _word_size{static_cast<dat::index>(_word.size())};
    ...
    for (dat::index _i = 0; _i < _word_size; ++_i)
    {
      const auto _maybe{
                ...
                dat::grid::layout_ite       p_to_position)
  {
    ...
    else
    {
      for (auto _coord : _intersects)
      {
        if (position_horizontally(p_grid, _coord, p_positioned, p_to_position))
        {
          return true;
        }
      }
