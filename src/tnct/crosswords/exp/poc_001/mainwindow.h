...
template <typename t> MainWindow<t>::~MainWindow() { delete ui; }
...
template <typename t> void MainWindow<t>::on_rbbVertical_clicked() {
  m_current_orientation = crosswords::mod::orientation::vert;
}

template <typename t> void MainWindow<t>::on_rdbHorizontal_clicked() {
  m_current_orientation = crosswords::mod::orientation::hori;
}

template <typename t> void MainWindow<t>::keyPressEvent(QKeyEvent *event) {
