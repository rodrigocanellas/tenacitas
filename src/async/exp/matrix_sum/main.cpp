...
using matrix = src::container::matrix<std::size_t, std::size_t>;
                                           ...
                                           std::string_view p_param)
{
  ...
  {
    TNCT_LOG_ERR(p_logger, src::format::fmt(p_param, " is not a valid number"));
  }
  return std::nullopt;
}
...
void sync_sum_matrix_wrapper(const matrix &p_matrix, logger &p_logger)
{
  ...
  TNCT_LOG_TST(p_logger, src::format::fmt("SYNC time = ", _diff.count(),
                                          " seconds, sum: ", _sum));
}
...
using dispatcher = src::async::dispatcher<logger, evt::matrix_summed>;
...
struct async_sum_matrix
{
  ...
  src::async::result operator()()
  {
    for (matrix::index _r = 0; _r < m_matrix.get_num_rows(); ++_r)
    {
      ...
      {
        TNCT_LOG_ERR(m_logger, src::format::fmt("error publishing 'sum_line': ",
                                                _result));
        return _result;
      }
    }
...
private:
  ...
  using internal_dispatcher =
      src::async::dispatcher<logger, sum_line, line_summed>;

  void define_handlers()
  {
    using sum_line_queue =
        src::container::circular_queue<logger, sum_line, 200>;
    m_internal_dispatcher.add_handling<sum_line>(
        ...
        src::async::handling_priority::medium, 8);

    using line_summed_queue =
        src::container::circular_queue<logger, line_summed, 10>;
  ...
  struct line_summer
  {
    ...
    void operator()(sum_line &&p_sum_line)
    {
      ...
      if (_result != src::async::result::OK)
      {
        TNCT_LOG_ERR(
            m_logger,
            src::format::fmt("error publishing 'line_summed': ", _result));
      }
    }
    ...
    void operator()(line_summed &&p_line_summed)
    {
      ...
      if (all_rows_summed())
      {
        ...
        if (_result != src::async::result::OK)
        {
          TNCT_LOG_ERR(
              m_logger,
              src::format::fmt("error publishing 'matrix_summed': ", _result));
        }
      }
    }
...
void async_sum_matrix_wrapper(matrix &p_matrix, logger &p_logger)
{
  ...
  using matrix_summed_queue =
      src::container::circular_queue<logger, evt::matrix_summed, 1>;
  ...
  TNCT_LOG_TST(p_logger, src::format::fmt("ASYNC time = ", _diff.count(),
                                          " seconds, sum: ", _sum));
}
// clang-format off
