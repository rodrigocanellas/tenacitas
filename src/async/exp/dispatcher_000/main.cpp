...
#include "tenacitas/src/async/exp/dispatcher_000/configuration.h"
#include "tenacitas/src/async/exp/dispatcher_000/event.h"
#include "tenacitas/src/async/exp/dispatcher_000/event_handled.h"
#include "tenacitas/src/async/exp/dispatcher_000/handler.h"
#include "tenacitas/src/async/exp/dispatcher_000/logger.h"
#include "tenacitas/src/async/exp/dispatcher_000/publisher.h"
#include "tenacitas/src/async/exp/dispatcher_000/results.h"
...
struct pgm
{
  using event_a       = src::async::exp::event<'a'>;
  using event_handled = src::async::exp::event_handled;

  using events_handled = std::tuple<event_a, event_handled>;

  int operator()(int argc, char **argv)
  {
    src::async::exp::logger _logger;
    ...
    try
    {
      ...
      src::async::exp::configuration _configuration(_logger, argv[1]);
      ...
      src::async::exp::results _results;
...
private:
  using logger = src::async::exp::logger;
  ...
  using publisher = src::async::exp::publisher<'a', dispatcher>;

  using queue_event_a = src::container::circular_queue<logger, event_a, 5000>;

  static constexpr size_t num_handlings{5};

  using handler_0 = src::async::exp::handler<'a', 0, dispatcher>;
  using handler_1 = src::async::exp::handler<'a', 1, dispatcher>;
  using handler_2 = src::async::exp::handler<'a', 2, dispatcher>;
  using handler_3 = src::async::exp::handler<'a', 3, dispatcher>;
  using handler_4 = src::async::exp::handler<'a', 4, dispatcher>;

  using results = src::async::exp::results;

  using configuration = src::async::exp::configuration<logger, num_handlings>;

  struct event_handled_handler
  {
    using events_handled = std::tuple<event_handled>;

    event_handled_handler(src::async::exp::logger &p_logger, size_t &p_total_handled,
                          src::async::exp::results     &p_results,
  ...
  private:
    src::async::exp::logger      &m_logger;
    size_t                  &m_total_handled;
    src::async::exp::results     &m_results;
    size_t                   m_percentage_to_display;
    size_t                   m_total_to_be_handled;
    std::condition_variable &m_cond_all_handled;
  };

  void define_handlings(dispatcher &p_dispatcher, src::async::exp::logger &p_logger,
