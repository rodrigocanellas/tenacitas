...
#ifndef TENACITAS_SRC_ASYNC_EXP_DISPATCHER_RESULTS_H
#define TENACITAS_SRC_ASYNC_EXP_DISPATCHER_RESULTS_H
...
#include "tenacitas/src/async/exp/dispatcher_000/event_id.h"
#include "tenacitas/src/async/exp/dispatcher_000/handler_id.h"
#include "tenacitas/src/async/exp/dispatcher_000/handler_type_id.h"
...
struct results
{
  void increment(event_id p_event_id, src::async::handling_name p_handling_name,
  ...
  std::string report() const
  {
    ...
    while (_ite != _end)
    {
      ...
      {
        src::async::handling_name _handling_prior{get_handling_name(_ite)};
...
private:
  using key   = std::tuple<event_id, src::async::handling_name, handler_type_id,
                         handler_id>;
  using value = size_t;
  ...
  using handlings_totals = std::map<src::async::handling_name, handling_total>;
...
private:
  void increment(events_totals &p_events_totals, event_id p_event_id,
                 const src::async::handling_name &p_handling_name,
                 handler_type_id p_handler_type_id, handler_id p_handler_id,
                 size_t p_value) const
  {
    ...
    handlings_totals::iterator _handling_ite{
        _event_ite->second.handlings.find(p_handling_name)};
    if (_handling_ite == _event_ite->second.handlings.end())
    {
      _handling_ite = _event_ite->second.handlings
                          .insert({p_handling_name, handling_total{}})
                          .first;
    }
  ...
  src::async::handling_name get_handling_name(map::const_iterator p_ite) const
  {
  ...
  size_t get_value(map::const_iterator p_ite) const
  {
    return p_ite->second;
  }
  ...
  const event_total &get_event_total(events_totals::const_iterator p_ite) const
  {
    return p_ite->second;
  }
  ...
  size_t get_handling_total_value(handlings_totals::const_iterator p_ite) const
  {
    return p_ite->second.total;
  }
