#ifndef TENACITAS_SRC_CROSSWORDS_DAT_CROSSWORDS_H
#define TENACITAS_SRC_CROSSWORDS_DAT_CROSSWORDS_H
...
#include "tenacitas/src/crosswords/dat/layout.h"
#include "tenacitas/src/crosswords/dat/occupied.h"
#include "tenacitas/src/crosswords/dat/orientation.h"
#include "tenacitas/src/crosswords/dat/permutation.h"
...
struct grid {
  ...
  grid(std::shared_ptr<const entries> p_entries,
      ...
      : m_longest(longest_word(p_permutation)), m_num_rows(p_num_rows),
        ...
        m_occupied(p_num_rows, p_num_cols, word::max_char),
        m_entries(p_entries) {
    ...
    if ((m_longest > p_num_rows) && (m_longest > p_num_cols)) {
      ...
      // TNCT_LOG_ERR(tenacitas::src::log::cerr,_err);
  ...
  const occupied &get_occupied() const { return m_occupied; }
...
private:
  void occupy(const_layout_ite p_layout) {
    ...
    if (p_layout->get_orientation() == orientation::vert) {
      for (index _i = 0; _i < p_layout->get_word().size(); ++_i) {
