...
struct test {
  ...
  /// \endcode
  ///
  /// \param argc is the number of strings in \p argv
  ///
  /// \param argv array of strings
  ///
  /// \details \p argv can contains "exec", which makes the test to be executed,
  /// or "desc", which makes <tt>t_class::desc()</tt> to be printed in \p cout
  ///
  ...
  template <typename t_class> static void run(int argc, char **argv) {
    ...
    if ((argc) == 1) {
      ...
      if (_param == "exec") {
        ...
        std::cout << "'" << _param << "' is not a valid parameter" << std::endl;
      }
    } else {
