...
#ifdef TENACITAS_LOG
    template <typename t_class, typename... t_params>
    void write(level p_level, t_class *p_this, const char *p_func, uint16_t p_line,
               const t_params &... p_params) {

        if (can_log(p_level)) {
            const uint16_t _str_size{5};
            ...
            char _this[2 + _str_size + 1];
            {
                ...
                std::string::size_type _size = _str.size();
                std::string::size_type _idx = _size - _str_size;
                for (std::string::size_type _i = 0; _i < _str_size; ++_i) {
            ...
            _func[m_max_func] = '\0';
            {
                decltype (m_max_func) _size =
                        static_cast<decltype (m_max_func)>(strlen(p_func));
                if (_size < m_max_func) {
            ...
            _stream << p_level << m_separator
                    << (m_timestamp_as_number
                        ...
                        : calendar::now<>::microsecs_str().substr(14, 25)) << m_separator
                    << _this  << m_separator
                    << m_class << m_separator
                    << _func << m_separator
                    << _th_id << m_separator
