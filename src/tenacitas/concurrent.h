...
///         if the callable object does not return void, \p execute
///         std::optional<type-returned> with value if not timeout, {} otherwise
///
/// \code
///
///#include <thread>
...
struct launcher {
    ...
    static std::optional<t_result> launch(timeout p_timeout, t_callable p_callable) {
        std::future<t_result> _future = std::async(std::launch::async, p_callable);
        if (_future.wait_for(p_timeout) != std::future_status::timeout) {
            return {_future.get()};
        }
        return {};
...
template <>
struct launcher<void> {
    template<typename t_callable>
    static bool launch(timeout p_timeout, t_callable p_callable) {
        std::future<void> _future = std::async(std::launch::async, p_callable);
        if (_future.wait_for(p_timeout) != std::future_status::timeout) {
            return true;
        }
        return false;
...
struct async_loop_t<t_log, t_time, use_breaker::yes, t_params...>
    ...
    typedef std::function<void(bool &,t_params...)> worker;
    ...
    void loop() override {
        ...
        while (true) {
            ...
            std::optional<bool> _maybe_break =
                    launcher<bool>::launch(this->m_breaker_timeout, m_breaker);
            if ((_maybe_break) && (*_maybe_break)) {
                break;
            }

            std::optional<std::tuple<t_params...>> _maybe_data =
                    launcher<std::tuple<t_params...>>::launch(this->m_provider_timeout, m_provider);

            if (this->m_stopped) {
                break;
            }

            _maybe_break = launcher<bool>::launch(this->m_breaker_timeout, m_breaker);
            if ((_maybe_break) && (*_maybe_break)) {
                break;
            }
            ...
            if (_maybe_data) {
                ...
                //                auto _worker = [this, &_abandon, _params]() -> void {
                //                    std::apply(m_worker, _abandon, _params);
                //                };

                if (!execute(this->m_timeout, m_worker, _abandon, _params)) {
                    _abandon = true;
...
struct async_loop_t<t_log, t_time, use_breaker::yes, void>
    ...
    typedef std::function<void(bool &)> worker;
...
protected:
    void loop() override {
        ...
        while (true) {
            ...
            bool _abandon{false};
            if (!execute(this->m_timeout, m_worker, _abandon)) {
                _abandon=true;
                this->launch_timeout_callback();
            }
        }
