...
protected:
  ...
  t_result call(std::function<t_result()> p_ok,
    ...
    {
      std::lock_guard<std::mutex> _lock(m_mutex_exec);
...
private:
  void loop(bool &p_abandoned) {
    while (true) {
      ...
      {
        std::lock_guard<std::mutex> _lock(m_mutex_wait);
        m_cond_wait.notify_one();
      }
