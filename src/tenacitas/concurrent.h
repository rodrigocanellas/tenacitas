...
struct id {
    explicit id(const std::string &p_value)
        : m_value(p_value) {}

    explicit id(std::string &&p_value)
        : m_value(std::move(p_value)) {}

    explicit id(const char *p_value)
        : m_value(p_value) {}
...
struct priority {
    ...
    explicit priority(uint8_t p_value=0)
        : m_value(p_value) {}
...
public:
    ...
    /// \brief
    template <typename t_time>
    worker_pool_t(const id & p_id, const priority &p_priority,
                   t_time p_timeout, on_timeout p_on_timeout)
        : m_impl(std::make_unique<implementation>(p_id, p_priority, p_timeout, p_on_timeout)){}
    ...
    /// \brief
     template <typename t_time>
     worker_pool_t(const id & p_id,
                    t_time p_timeout, on_timeout p_on_timeout)
     : m_impl(std::make_unique<implementation>(p_id, p_timeout, p_on_timeout))
     {}
     ...
     /// \brief
      ...
      /// \brief
      template <typename t_time>
      worker_pool_t(const priority &p_priority,
                     t_time p_timeout, on_timeout p_on_timeout)
      : m_impl(std::make_unique<implementation>(p_priority, p_timeout, p_on_timeout)){}
      ...
      /// \brief
       template <typename t_time>
       worker_pool_t(t_time p_timeout, on_timeout p_on_timeout)
       : m_impl(std::make_unique<implementation>(p_timeout, p_on_timeout)) {}

       /// \brief
        template <typename t_time>
        worker_pool_t( t_time p_timeout)
    ...
    /// \brief
    const priority & get_priority () const {return m_impl->get_priority();}

    /// \brief
    bool operator < (const worker_pool_t& p_worker_pool) const {
        return m_impl->get_priority() < p_worker_pool.m_impl->get_priority();
    }
    ...
    /// \brief
    bool operator != (const worker_pool_t& p_worker_pool) const {
        return m_impl->get_id() != p_worker_pool.m_impl->get_id();
    }
...
private:
    struct implementation {
        /// \brief
        template <typename t_time>
        implementation(const id & p_id, const priority &p_priority,
                       t_time p_timeout, on_timeout p_on_timeout)
        ...
        /// \brief
         ...
         /// \brief
          ...
          /// \brief
          template <typename t_time>
          implementation(const priority &p_priority,
                         t_time p_timeout, on_timeout p_on_timeout)
              : m_id(std::to_string(uuid())), m_priority(p_priority), m_queue(10),
                m_timeout(internal::to_timeout(p_timeout)),
                m_on_timeout(p_on_timeout) {}
        ...
        /// \brief
        const priority & get_priority () const {return m_priority;}

        /// \brief
        bool operator < (const implementation & p_impl) const {
...
//    inline static void
//    traverse(std::function<void(const pool_id &, priority, timeout)> p_visitor) {
...
//        /// \brief
//        inline timeout get_timeout() const { return m_worker_pool.get_timeout(); }
...
//    static void find_handle(const pool_id &p_pool_id,
//                            std::function<void(item &)> p_cb) {
//        auto _cmp = [&p_pool_id](const item &p_item) -> bool {
//            return p_pool_id == p_item.get_id();
//        };
//        auto _end = m_itens.end();
