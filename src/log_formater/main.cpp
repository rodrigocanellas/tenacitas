...
struct log_formater {
    ...
    ///
    /// \details Parameters passed to the program must be '--in', with the name
    /// of the input file, containing log messages, and '--out', which will be
    /// the name of the output file, containg formated log  messages
  ...
  private:
    void output() {
        ...
        //    auto _this = [this](const std::string &p_line,
        //    std::string::size_type p_1,
        //                        std::string::size_type p_2) -> void {
        //      const uint8_t _size = static_cast<uint8_t>(p_2 - p_1) - 2; // 0x
        //      if (_size >= m_this_size) {
        //        m_out << "0x" << p_line.substr(p_2 - m_this_size,
        //        m_this_size);
        //      } else {
        ...
        auto _thread_id = [this](const std::string &p_line,
            ...
            if (_size >= m_thread_id_size) {
                m_out << p_line.substr(p_2 - m_thread_id_size,
                                       m_thread_id_size);
            } else {
        ...
        auto _msg = [this](const std::string &p_line,
                           std::string::size_type p_1,
                           std::string::size_type p_2) -> void {
            m_out.write(p_line.c_str() + p_1, p_2 - p_1);
            m_out << std::endl;
        ...
        while (true) {
            ...
            if (!parse_line(_line, _log_level, _timestamp,
                            /*_this,*/ _class_name, _function_name, _thread_id,
                            _line_number, _msg)) {
                return;
            }
