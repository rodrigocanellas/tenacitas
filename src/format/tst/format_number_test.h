...
#ifndef TENACITAS_SRC_FORMAT_TST_FORMAT_NUMBER_H
#define TENACITAS_SRC_FORMAT_TST_FORMAT_NUMBER_H
...
namespace tenacitas::src::format::tst
{
...
struct format_number_001
{
  ...
  {
    const auto _str(
        src::format::format_max_number(static_cast<int16_t>(-34), ' '));
...
struct format_number_003
{
  ...
  {
    const auto _str(
        src::format::format_max_number(static_cast<int8_t>(-4), ' '));
...
struct format_number_006
{
  ...
  {
    // const std::string
    // _str(src::format::format_number<std::string>("-3"),'0');
    return true;
  }
};
...
struct format_number_010
{
  ...
  {
    const auto _str(
        src::format::format_max_number(static_cast<int32_t>(-34), ' '));
    std::cerr << "str = '" << _str << '\'' << std::endl;
    return _str == "       -34";
  }
};
...
struct format_number_012
{
  ...
  {
    const auto _str(
        src::format::format_max_number(static_cast<int64_t>(-34), ' '));
    std::cerr << "str = '" << _str << '\'' << std::endl;
    return _str == "                 -34";
  }
};
...
struct format_number_014
{
  ...
  {
    const auto _str(
        src::format::format_fix_number(static_cast<uint64_t>(34), 6));
    std::cerr << "str = '" << _str << '\'' << std::endl;
    return _str == "000034";
  }
};
...
struct format_number_015
{
  ...
  {
    const auto _str(
        src::format::format_fix_number(static_cast<uint32_t>(73290), 4));
    std::cerr << "str = '" << _str << '\'' << std::endl;
    return _str == "73290";
  }
};
