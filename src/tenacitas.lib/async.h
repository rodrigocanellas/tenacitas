    ...
    namespace async {
        ...
        namespace internal {
            ...
            struct executer_t {
                ...
                std::optional<t_ret> operator()(t_time p_timeout, t_function& p_function, t_params&&... p_params) {
                    std::mutex _mutex;
            ...
            struct executer_t<void> {
                template<typename t_time, typename t_function, typename... t_params>
                bool operator()(t_time p_timeout, t_function& p_function, t_params&&... p_params) {
                    std::mutex _mutex;
                    std::condition_variable _cond;
        ...
        template<typename t_time, typename t_function, typename... t_params>
        typename result_traits<std::invoke_result_t<t_function, type::ptr<bool>, t_params...>>::result
        execute(t_time p_timeout, t_function& p_function, t_params&&... p_params) {
            typedef internal::executer_t<std::invoke_result_t<t_function, type::ptr<bool>, t_params...>> executer;
            executer _executer;
            return _executer(p_timeout, p_function, std::forward<t_params>(p_params)...);
        }
        ...
        namespace internal {
            ...
            struct circular_unlimited_size_queue_t {
                ...
                circular_unlimited_size_queue_t(const circular_unlimited_size_queue_t&) = delete;
                ...
                circular_unlimited_size_queue_t& operator=(const circular_unlimited_size_queue_t&) = delete;

                // \brief Move assignment
                circular_unlimited_size_queue_t& operator=(circular_unlimited_size_queue_t&& p_queue) {
                    if (this != p_queue) {
                        m_root = p_queue.m_root;
                        m_write = p_queue.m_write;
                        m_read = p_queue.m_read;
            ...
            public:
                ...
                handling_t(t_time p_timeout, priority p_priority = 125) :
                    m_timeout(calendar::convert<timeout>(p_timeout)), m_priority(p_priority), m_queue(1) {}
                ...
                inline size_t amount_added() const {
                ...
                void add_handler(handler&& p_handler) {
                    ...
                    m_loops.push_back(std::thread(&handling_t<t_event>::handler_loop, this, std::move(p_handler)));
                    m_stopped = false;
                }
                ...
                void add_handler(uint16_t p_num_workers, std::function<handler()> p_factory) {
                ...
                inline const handling_id& get_id() const {
                    return m_id;
                }

                // \return The tenacitas::async::priority of this handling
                inline const priority& get_priority() const {
                    return m_priority;
                }

                // \brief Sets the tenacitas::async::priority of this handling
                inline void set_priority(priority p_priority) {
            ...
            private:
                ...
                void handler_loop(handler&& p_handler) {
                    number::id _loop_id;
                    TRA(m_id, ':', _loop_id, " - entering loop, m_timeout = ", this->m_timeout.count());
            ...
            struct dispatcher_t {
                ...
                template<typename t_time>
                static inline handling_id add_handling(t_time p_timeout = 5s, priority p_priority = 125) {
                    std::lock_guard<std::mutex> _lock(m_mutex);
                ...
                // \return a \p handling_id, identinfying the handling
                template<typename t_time>
                static handling_id add_handling(handler&& p_handler, t_time p_timeout = 5s, priority p_priority = 125) {
                ...
                static void send(const t_event& p_event) {
                    for (handling_ptr& _handling_ptr : m_list) {
                        INF(get_id(), " - sending ", p_event, " to pool ", _handling_ptr->get_id());
                ...
                //
                // \param p_handler is the handler function to be added
                static void add_handler(const handling_id& p_handling_id, handler&& p_handler) {
                ...
                static void traverse(
                    std::function<void(const handling_id&, priority, const std::chrono::milliseconds&)> p_visitor) {
                    for (const handling_ptr& _handling_ptr : m_list) {
                        p_visitor(
                            _handling_ptr->get_id(),
                            _handling_ptr->get_priority(),
                            _handling_ptr->template get_timeout<std::chrono::milliseconds>());
                    }
                }
                ...
                static number::id get_id() {
                    return event_id_t<t_event>::value;
                }
            ...
            private:
                ...
                static inline void sort() {
                    m_list.sort([](const handling_ptr& p_i1, const handling_ptr& p_i2) -> bool {
        ...
        struct sleeping_loop_t {
            ...
            sleeping_loop_t(function p_function, t_timeout p_timeout, t_interval p_interval) :
        ...
        private:
            ...
            void loop() {
                ...
                while (true) {
                    ...
                    TRA("waiting for ", m_interval.count(), " ms to elaps, or a stop order");
                    {
                        std::unique_lock<std::mutex> _lock(m_mutex_interval);
                        if ((m_cond.wait_for(_lock, m_interval) == std::cv_status::no_timeout) && (m_stopped)) {
                            TRA("ordered to stop");
                            break;
                        }
                    }
        ...
        /// \return the handling id of the handling added
        template<typename t_event, typename t_time>
        static inline handling_id
        add_handling(handler_t<t_event>&& p_handler, t_time p_timeout = 5s, priority p_priority = 125) {
            return internal::dispatcher_t<t_event>::add_handling(std::move(p_handler), p_timeout, p_priority);
        }
        ...
        template<typename t_event>
        static inline void set_priority(const handling_id& p_handling, priority p_priority) {
        ...
        /// found
        template<typename t_event>
        static inline std::optional<priority> get_priority(const handling_id& p_handling) {
        ...
        /// handling
        template<typename t_event>
        static inline void add_handler(const handling_id& p_handling, handler_t<t_event>&& p_handler) {
            internal::dispatcher_t<t_event>::add_handler(p_handling, std::move(p_handler));
        }
        ...
        /// \param p_factory is a function that creates handler function
        template<typename t_event>
        static inline void add_handler(
            const handling_id& p_handling,
            uint16_t p_num_workers,
            std::function<handler_t<t_event>()> p_factory) {
            internal::dispatcher_t<t_event>::add_handler(p_handling, p_num_workers, p_factory);
        }
        ...
        template<typename t_event, typename t_time>
        static inline handling_id
        add_handler(handler_t<t_event>&& p_handler, t_time p_timeout, priority p_priority = 125) {
        ...
        struct reader_t {
            ...
            /// \bried Creates an asynchronous reader
            ///
            ...
            template<typename t_time>
            reader_t(sync_reader p_reader, t_time p_timeout) :
                m_reader(p_reader), m_handling_id(add_handling<incoming>(p_timeout)) {}

            template<typename t_time>
            reader_t(sync_reader p_reader, t_time p_timeout, handler_t<t_data>&& p_handler) :
                m_reader(p_reader), m_handling_id(add_handling<incoming>(p_timeout)) {
                add_handler(m_handling_id, std::move(p_handler));
            }

            void add_handler(handler_t<t_data>&& p_handler) {
                add_handler(m_handling_id, std::move(p_handler));
            }

            template<typename t_number>
            void add_handlers(t_number p_num_handler, std::function<handler_t<t_data>()>& p_factory) {
                for (auto _i = static_cast<t_number>(0); _i < p_num_handler; ++_i) {
                    add_handler(m_handling_id, std::move(p_factory()));
                }
            }
            ...
            void start() {
                ...
                m_thread = std::thread([this]() -> void {
                    while (true) {
                        ...
                        dispatch(incoming {std::move(*_maybe)});
                    }
                });
            }
            ...
            void stop() {
                ...
                m_stopped = true;
                if (m_thread.joinable()) {
                    if (!execute(1s, [this](type::ptr<bool>) -> void { return m_thread.join(); })) {
                        m_thread.detach();
                    }
                }
            }
        ...
        private:
            struct incoming {
                incoming(t_data&& p_data) : data(std::move(p_data)) {}
                friend std::ostream& operator<<(std::ostream& p_out, const incoming& /*p_incoming*/) {
                    p_out << "incoming";
                    return p_out;
                }
                t_data data;
        ...
        private:
            /// \brief Actual reader object, that will retrieve a \p t_data from some source
            sync_reader m_reader;

            /// \brief Handling id for the asynchronous handlers
