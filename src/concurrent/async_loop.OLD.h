...
/// \brief Base class to define a loop that runs asynchronously
///
/// \tparam t_log
template <typename t_log> struct async_loop_base_t {
  ...
  /// one of the time units defined in std::chrono
  ///
  /// \param p_timeout is the amount of time a worker has to execute,
  /// before the caller looses interest in its result
  template <typename t_time>
  async_loop_base_t(t_time p_timeout) : m_timeout(to_timeout(p_timeout)) {}
  ...
  ///
  /// \param p_timeout is the new value of the timeout
  ///
...
protected:
  /// amount of time a worker has to execute, before the caller looses interest
  /// in its result
  timeout m_timeout;
