...
namespace tenacitas::src::parser::tst
{
...
struct test_000
{
  ...
  bool operator()(const src::program::options &p_options)
  {
    ...
    try
    {
      ...
      {
        m_logger.err(
            src::format::fmt("no sections found in '", _ini_file_name, '\''));
        return false;
      }
...
int main(int argc, char **argv)
{
  ...
  src::tester::test _test(argc, argv, {"ini"});
  run_test(_test, src::parser::tst::test_000);
}
