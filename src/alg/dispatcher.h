...
#ifndef TENACITAS_LIB_ALG_DISPATCHER_H
#define TENACITAS_LIB_ALG_DISPATCHER_H
...
#include <tenacitas.lib/src/cpt/publisher.h>
#include <tenacitas.lib/src/cpt/subscriber.h>
#include <tenacitas.lib/src/dat/event_handler.h>
#include <tenacitas.lib/src/dat/event_priority.h>
#include <tenacitas.lib/src/dat/internal/event_queue.h>
...
It is possible to define a tld::event_priority between the queues, so that an
event is delivered to the highest tld::event_priority queue before the others.
...
*/
template <tlc::event... t_events> struct dispatcher {
  ...
  tld::queue_id
  add_queue(tld::event_priority p_priority = tld::event_priority ::medium) {
    ...
    tld::queue_id _queue_id{_ite->get_id()};
  ...
  template <tlc::subscriber t_subscriber, tlc::event t_event>
  tld::queue_id
  subscribe(tld::event_handler<t_event> p_subscriber,
            tld::event_priority p_priority = tld::event_priority ::medium) {
  ...
  template <tlc::subscriber t_subscriber, tlc::event t_event>
  void subscribe(const tld::queue_id &p_queue_id,
  ...
  template <tlc::event t_event> void clear() {
    constexpr auto _event_index{assert_event_defined_in_dispatcher<t_event>()};
  ...
  template <tlc::event t_event> void clear(tld::queue_id p_queue_id) {

    constexpr auto _event_index{tlt::tuple_index_v<t_event, events>};
  ...
  template <tlc::publisher t_publisher, tlc::event t_event>
  ...
  template <tlc::publisher t_publisher, tlc::event t_event>
  void publish_to_queue(tld::queue_id p_queue_id, const t_event &p_event) {
  ...
  template <tlc::event t_event>
  void set_priority(const tld::queue_id &p_queue_id,
                    tld::event_priority p_priority) {
    constexpr auto _event_index{assert_event_defined_in_dispatcher<t_event>()};
  ...
  template <tlc::event t_event>
  tld::event_priority get_priority(const tld::queue_id &p_queue_id) {
    constexpr auto _event_queues_list_index{
        assert_event_defined_in_dispatcher<t_event>()};
  ...
  template <tlc::event t_event>
  size_t queue_size(const tld::queue_id &p_queue_id) {
  ...
  template <tlc::event t_event>
  size_t occupied_in_queue(const tld::queue_id &p_queue_id) {
    constexpr auto _event_queues_list_index{
        assert_event_defined_in_dispatcher<t_event>()};
  ...
  template <tlc::event t_event> size_t amount_of_queues() const {
    constexpr auto _event_queues_list_index{
        assert_event_defined_in_dispatcher<t_event>()};
  ...
  //  template <tlc::event t_event> void empty_queue();

  //  /// \brief Removes all events in all the queues of \p t_event
  //  template <tlc::event t_event> void empty_queue(queue_id p_queue_id);
...
private:
  ...
  using event_queue = tld::internal::event_queue<t_event>;

  template <typename t_event>
  using event_queue_list = std::list<event_queue<t_event>>;

  using event_queue_list_tuple =
      std::tuple<std::list<tld::internal::event_queue<t_events>>...>;

  template <typename t_event>
  using event_queue_list_iterator =
      typename event_queue_list<t_event>::iterator;

  template <tlc::event t_event> struct event_mutex { std::mutex mutex; };
...
private:
  ...
  template <size_t t_event_index, tlc::event t_event>
  event_queue_list_iterator<t_event>
  find_event_queue(tld::queue_id p_queue_id) {
  ...
  static constexpr size_t assert_event_defined_in_dispatcher() {
    constexpr auto _maybe_index{tlt::tuple_index_v<t_event, events>};
    static_assert(_maybe_index,
                  "trying to subscribe to an event not defined in dispatcher");
    return static_cast<size_t>(_maybe_index.value());
  }
  ...
  static constexpr void assert_event_defined_in_subscriber() {
    ...
    static_assert(
        tlt::tuple_index_v<t_event, events_subscribed>,
  ...
  constexpr void assert_event_defined_in_publisher() const {
    ...
    static_assert(tlt::tuple_index_v<t_event, events_published>,
