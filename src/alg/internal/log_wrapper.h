...
#ifndef TENACITAS_LIB_ALG_INTERNAL_LOG_WRAPPER_H
#define TENACITAS_LIB_ALG_INTERNAL_LOG_WRAPPER_H
...
#include <tenacitas.lib/src/dat/internal/max_log_file_name_lenght.h>
#include <tenacitas.lib/src/dat/log_level.h>
#include <tenacitas.lib/src/tra/is_tuple.h>
...
public:
  ...
  inline static void test(const char *p_log_file, uint16_t p_line,
                          t_params &&...p_params) {
    write(tld::log_level::test, p_log_file, p_line,
  ...
  inline static void trace(const char *p_log_file, uint16_t p_line,
                           t_params &&...p_params) {
    if (tld::log_level::trace >= g_level) {
      write(tld::log_level::trace, p_log_file, p_line,
            std::forward<t_params>(p_params)...);
    }
  }
  ...
  inline static void debug(const char *p_log_file, uint16_t p_line,
                           t_params &&...p_params) {
    if (tld::log_level::debug >= g_level) {
      write(tld::log_level::debug, p_log_file, p_line,
            std::forward<t_params>(p_params)...);
    }
  }
  ...
  inline static void info(const char *p_log_file, uint16_t p_line,
                          t_params &&...p_params) {
    if (tld::log_level::info >= g_level) {
      write(tld::log_level::info, p_log_file, p_line,
            std::forward<t_params>(p_params)...);
    }
  }
  ...
  inline static void warn(const char *p_log_file, uint16_t p_line,
                          t_params &&...p_params) {
    if (tld::log_level::warn >= g_level) {
      write(tld::log_level::warn, p_log_file, p_line,
            std::forward<t_params>(p_params)...);
    }
  }
  ...
  inline static void error(const char *p_log_file, uint16_t p_line,
                           t_params &&...p_params) {
    write(tld::log_level::error, p_log_file, p_line,
          std::forward<t_params>(p_params)...);
  }
  ...
  inline static void fatal(const char *p_log_file, uint16_t p_line,
                           t_params &&...p_params) {
    write(tld::log_level::fatal, p_log_file, p_line,
          std::forward<t_params>(p_params)...);
  }

  inline static void set_trace() { g_level = tld::log_level::trace; }
  inline static void set_debug() { g_level = tld::log_level::debug; }
  inline static void set_info() { g_level = tld::log_level::info; }
  inline static void set_warn() { g_level = tld::log_level::warn; }
...
private:
  ...
  /// \param p_params are the contens of the log message
  template <typename... t_params>
  static void write(tld::log_level p_level, const char *p_log_file,
    ...
    _stream << p_level << m_separator << _time_str << ','
            ...
            << std::left << std::setw(tld::internal::max_log_file_name_lenght)
  ...
  static void format(std::ostream &p_stream, const t &p_t) {
    if constexpr (tlt::is_tuple<std::decay_t<t>>::value) {
      format(p_stream, '(');
...
/// \brief Current log level
template <bool use>
tld::log_level log_wrapper<use>::g_level{tld::log_level::warn};
} // namespace tenacitas::lib::alg::internal
