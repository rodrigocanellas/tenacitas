...
constexpr inline months convert<seconds, months>(seconds p_seconds) {
  //  return convert<seconds, days>(p_seconds.value()) / static_cast<double>(7);
  return months(p_seconds.value() /
                static_cast<double>(60 * 60 * 24 * 365 / 12));
}
...
constexpr inline months convert<minutes, months>(minutes p_minutes) {
  return months(p_minutes.value() / static_cast<double>(60 * 24 * 365 / 12));
}
...
template <> constexpr inline months convert<hours, months>(hours p_hours) {
  return months(p_hours.value() / static_cast<double>(24 * 365 / 12));
}
...
// template <> constexpr inline minutes convert<hours, minutes>(hours p_hours) {
//  return minutes(p_hours.value() * 60);
//}
...
// template <> constexpr inline minutes convert<months, minutes>(months
// p_months) {
//  return convert<hours, minutes>(
//      convert<days, hours>(convert<months, days>(p_months)));
//}

// template <> constexpr inline seconds convert<months, seconds>(months
// p_months) {
//  return convert<minutes, seconds>(convert<hours, minutes>(
//      convert<days, hours>(convert<months, days>(p_months))));
//}
