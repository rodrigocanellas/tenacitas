#ifndef TENACITAS_MEASURES_WEEKDAY_H
#define TENACITAS_MEASURES_WEEKDAY_H
...
struct weekday {
  ...
  /// a \p weekday object
  ///
  template <typename t_int> inline explicit weekday(t_int p_value) {
  ...
  ///
  explicit inline weekday(amount_t<weekday> p_amount)
  ...
  ///
  inline weekday &operator=(amount_t<weekday> p_amount) {
...
/////
//  days operator-(const weekday &p_left, const weekday &p_right) {
//  if (p_left > p_right) {
...
/////
// inline  void operator++(weekday &p_w) {
...
///// \param p_w weekday to be advanced
/////
// inline  void operator++(weekday &p_w, int) {
//  p_w = (p_w == weekday::sat ? weekday::sun
//                             : weekday(static_cast<uint8_t>(p_w) + 1));
//}
