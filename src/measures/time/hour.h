#ifndef TENACITAS_MEASURES_HOUR_H
#define TENACITAS_MEASURES_HOUR_H
...
struct hour {
  ...
  /// a \p hour object
  ///
  template <typename t_int> explicit hour(t_int p_value) { assign(p_value); }

  ///
  /// \brief hour constructor from a \p amount_t<hour>, i.e., \p hours
  /// \param p_amount amount of hours used to create a \p hour
  ///
  explicit inline hour(amount_t<hour> p_amount) : hour(p_amount.value()) {}
  ...
  /// \param p_amount amount of hours used to create a \p hour
  ///
  inline hour &operator=(amount_t<hour> p_amount) {
  ...
  /// \return
  ///
  inline bool operator>(const hour &p_hour) const {
...
/// \return
///
inline hour operator"" _hr(unsigned long long p_int) { return hour(p_int); }
