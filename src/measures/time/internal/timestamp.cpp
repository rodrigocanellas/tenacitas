...
timestamp::timestamp(year p_year, month p_month, day p_day, hour p_hour,
 ...
 void timestamp::dMyhms2days_secs(year p_year, month p_month,
                                           day p_day, hour p_hour,
                                           minute p_minute, second p_second) {

  int64_t _years = p_year.value<int64_t>();
  const decltype(_years) _months = p_month.value<decltype(_months)>();
  const decltype(_years) _days = p_day.value<decltype(_days)>();
  _years -= _months <= 2;
  ...
  //  m_secs = seconds(static_cast<int64_t>(m_secs));
}

 void timestamp::days_secs2dMywhms() {
