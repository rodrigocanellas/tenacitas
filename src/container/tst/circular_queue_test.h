...
namespace tenacitas::src::container::tst
{
...
struct circular_queue_001
{
  ...
  bool operator()(const src::program::options &)
  {
    ...
    try
    {
      ...
      src::container::circular_queue<src::log::cerr, std::string,
                                     m_initial_size>
                  queue(_logger);
      ...
      while (!queue.empty())
      {
        ...
        if (!maybe)
        {
          _logger.err(src::format::fmt("error getting data # ", j));
          return false;
        }
        _logger.tst(src::format::fmt("capacity = ", queue.capacity(),
...
struct circular_queue_003
{
  ...
  bool operator()(src::program::options &)
  {
    using queue = src::container::circular_queue<src::log::cerr, int32_t, 30>;
    src::log::cerr _logger;
...
struct circular_queue_test
{
  ...
  bool operator()(src::program::options &p_options)
  {
    bool           _success(true);
    src::log::cerr _logger;
    try
    {
      ...
      if (!_maybe_sections)
      {
        _logger.tst(src::format::fmt("Failed to read sections from ini file ",
                                     _ini_file_name));
        return false;
      }
      ...
      if (_sections.size() == 0)
      {
        _logger.tst(src::format::fmt("No sections read from ini file ",
                                     _ini_file_name));
        return false;
      }
      ...
      {
        _logger.tst(src::format::fmt("############ -> ", _value_section.first));
        ...
        if (!_run_all_tests
            && (std::find_if(_tests_to_run.begin(), _tests_to_run.end(),
                ...
                == _tests_to_run.end()))
        {
          _logger.tst(src::format::fmt("Test ", _value_section.first,
        ...
        else
        {
          ...
          if (parse(_value_section.second, _steps, _logger))
          {
            ...
            {
              _logger.tst(src::format::fmt(_value_section.first, " FAIL"));
              _success = false;
            }
            else
            {
              _logger.tst(src::format::fmt(_value_section.first, " SUCCESS"));
            }
          }
          else
          {
            _logger.err(src::format::fmt("error parsing section ",
...
private:
  ...
  using ini_file = src::parser::ini_file<logger>;
  ...
  struct step_type
  {
    ...
    bool parse(const std::string &p_step)
    {
      ...
      if (_num_matches != 9)
      {
        m_logger.tst(src::format::fmt("# matches in step ", p_step,
      ...
      {
        m_logger.err(src::format::fmt("capacity expected = ", capacity_expected,
                                      ", but ", contents.size(),
                                      " items were found"));
        return false;
      }
  ...
  using queue = src::container::circular_queue<logger, int32_t, 8>;
...
private:
           ...
           logger &p_logger)
  {
    ...
    for (const auto &_step : p_steps)
    {
      ...
      {
        p_logger.tst(src::format::fmt("step ", ++_step_counter, ": ",
                                      _queue.brief_report()));
      }
           ...
           _i < _step.capacity_expected; ++_i)
      {
        ...
        {
          p_logger.err(src::format::fmt("content is ", _queue[_i], ", but ",
    ...
    for (const auto &_property : p_properties)
    {
      ...
      if (std::regex_match(_key, _base_match, _regex))
      {
        if (_base_match.size() == 2)
        {
          ...
          {
            p_logger.tst(src::format::fmt("error parsing step '", _key, '\''));
            return false;
          }
        }
      }
