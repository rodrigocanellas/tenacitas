...
*/
  ...
  bool operator()(const program::options &p_options) {
    ...
    try {
      ...
      const size_t _num_events_to_be_handled{calculate_num_events_to_be_handled(
      ...
      handle_events_handled(_dispatcher, _logger, _amount_events_handled,
                            _num_events_to_be_handled, _cond, _events_handled);
      ...
      TNCT_LOG_TST(_logger,
                   format::fmt(_num_events_to_be_handled,
                               ...
                               _amount_events_handled, " events were handled"));
...
private:
  void handle_events_published(dispatcher &p_dispatcher, logger &p_logger,
    ...
    auto _result{p_dispatcher.subscribe<event_published, event_published_queue,
                                        decltype(_handle_event_published)>(
        std::move(_handle_event_published), event_published_queue{p_logger})};
  ...
  void report(const events_handled &p_events_handled, logger &p_logger) {
    ...
    for (const events_handled::value_type &_value : p_events_handled) {
      _stream << "handling " << _value.first << ", handled = " << _value.second
              << " events" << std::endl;
    }
  ...
  size_t calculate_num_events_to_be_handled(
    ...
    for (size_t _i = 0; _i < _cfgs; ++_i) {
      if (p_handlings_cfg[_i].use && p_publishers_cfg[_i].use) {
        ...
        TNCT_LOG_TST(p_logger, format::fmt("handling ", p_handlings_cfg[_i].id,
                                           " will handled ", _aux, " events"));
        _result += _aux;
      }
    }
    return _result;
  ...
  bool operator()(const program::options &p_options) {
    ...
    try {
      ...
      handlings_cfg _handlings_cfg;
      {
        ...
        if (!_maybe_cfg) {
          TNCT_LOG_ERR(_logger, format::fmt("error reading ", _ini_file_name));
          return false;
        }

        std::pair<publishers_cfg, handlings_cfg> _pair_cfg{_maybe_cfg.value()};
...
private:
  ...
  //   event_handled() = default;
  //   event_handled(const handling_id &p_handling, const handler_id &p_handler)
  ...
  //   using events_totals =
  //       std::tuple<event_total<ev_a>, event_total<ev_b>, event_total<ev_c>,
  ...
  struct publisher_cfg {
    ...
    friend std::ostream &operator<<(std::ostream &p_out,
      ...
      p_out << '(' << p_publisher_cfg.id << ','
            << (p_publisher_cfg.use ? 'Y' : 'N') << ',' << p_publisher_cfg.event
