  ...
  */
  ...
  bool operator()(const program::options &p_options) {
    ...
    subscribers _subscribers(
        subscriber<0>(_dispatcher, _logger, _amount_by_handling_by_subscriber),
        subscriber<1>(_dispatcher, _logger, _amount_by_handling_by_subscriber),
        subscriber<2>(_dispatcher, _logger, _amount_by_handling_by_subscriber),
        subscriber<3>(_dispatcher, _logger, _amount_by_handling_by_subscriber),
        subscriber<4>(_dispatcher, _logger, _amount_by_handling_by_subscriber));
    ...
    _logger.tst(generic::fmt(_amount_events_handled, " events were handled"));
    ...
    for (const auto &_value : _amount_by_handling_by_subscriber.get_map()) {
      ...
      _logger.tst(generic::fmt("handling ", _handling_id, " subscriber ",
                               _value.first.second, " handled ", _value.second,
                               " events"));
...
private:
  ...
  struct publisher_cfg {
    ...
    publisher_cfg(std::string_view p_id, bool p_use,
        ...
        : id(p_id), use(p_use), interval(p_interval), num_events(p_num_events) {
    }
  ...
  struct handling_cfg {
    ...
    handling_cfg(std::string_view p_id, bool p_use, uint16_t p_num_subscribers,
  ...
  struct publisher {
    ...
    publisher(std::string_view p_id, logger &p_logger, dispatcher &p_dispatcher,
  ...
  struct event_handled_subscriber {
    ...
    event_handled_subscriber(std::condition_variable &p_cond,
                             size_t &p_amount_events_handled, logger &p_logger)
    ...
    void handle(event_handled &&) {
      ...
      if ((m_amount_events_handled % 10) == 0) {
        m_logger.tst(generic::fmt(m_amount_events_handled, " events handled"));
      }
      m_cond.notify_all();
    }
...
private:
  ...
  bool create_subscribers(const handlings_cfg &p_handlings_cfg,
                          subscribers &p_subscribers, dispatcher &p_dispatcher,
                          logger &p_logger) {
