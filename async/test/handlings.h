...
struct handlings_002 : public handlings_base {
  ...
  bool operator()(const program::options &) {
    ...
    _log.tst(generic::fmt("total handled by ", _handling_id_1, " = ", _total));
    ...
    _log.tst(generic::fmt("total handled by ", _handling_id_2, " = ", _total));

    return (_counter_1 == m_amount_events) && (_counter_2 == m_amount_events);
  }
