...
struct insert_definition
{
  ...
  inline columns_values::const_iterator
  add_column_value(const column_name & p_column_name
                   , column_value_pattern p_column_value_pattern) {
    return m_columns_values.add(p_column_name, p_column_value_pattern);
  }

  inline foreigners_keys_values::const_iterator
  add_foreign_key_value(const foreign_key_name & p_foreign_key_name
                        , foreign_key_value_pattern p_foreign_key_value_pattern
                        , column_value_pattern p_column_value_pattern) {
    return m_foreigners_keys_values.add(p_foreign_key_name
                                        , p_foreign_key_value_pattern
                                        , p_column_value_pattern);
  }
  ...
  inline const database_name &get_database_name()const{return m_database_name;}
  ...
  inline const foreigners_keys_values::const_iterator begin_foreign_key_value()const {
    return m_foreigners_keys_values.begin();
  }

  inline const foreigners_keys_values::const_iterator end_foreign_key_value()const {
