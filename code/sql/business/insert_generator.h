...
private:
  ...
  std::string generate_table_insert(const hosts p_hosts
        ...
        for(uint16_t _row = 0; _row < _amount_of_rows; ++_row) {
          _stream << generate_row_insert(_table, _row, p_insert_definition) << std::endl;
        }

        return _stream.str();
  }

  std::string generate_row_insert(ptr<table> p_table
                                  , uint16_t p_row
                                  , const insert_definition & p_insert_definition) {
    std::stringstream _stream;
  ...
  //  std::string generate_table_insert_for_table(table* p_table, uint16_t p_num_lines)
  //  {
  //    std::stringstream _stream;
  ...
  //  }
                          ...
                          const insert_definition& p_insert_definition) const
    {
      ...
      ptr<database> _database = _server->find(p_insert_definition.get_database_name());
