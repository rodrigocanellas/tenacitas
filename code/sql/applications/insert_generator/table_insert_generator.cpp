...
TableInsertGenerator::on_btnGenFks_clicked()
{
  ...
  for (int _row_def = 0; _row_def < _num_rows_def; ++_row_def) {
    ...
    if (_generator) {
      m_table_generator->add_fk_generator(_col_name.toStdString(), _generator);
    }
  }
...
TableInsertGenerator::on_pk_generated(std::string p_table_name)
{
  ...
  int _num_lines = ui->lstMissingPks->count();
  for (int _count_line = 0; _count_line < _num_lines; ++_count_line) {
