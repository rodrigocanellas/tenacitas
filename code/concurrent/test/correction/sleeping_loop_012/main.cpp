...
sleeping_loop_t<msg, logger::business::log>
loop;
...
struct sleeping_loop_012
{
  ...
  bool operator()()
  {
    ...
    concurrent_log_test(logger::business::log, "stopping");
    _loop_1.stop();
    _loop_2.stop();
    _loop_3.stop();

    concurrent_log_test(logger::business::log, "data 1 = ", _work_1.m_msg);
    if ((_work_1.m_msg.m_i != 10) && (_work_1.m_msg.m_f != 1024.0)) {
      return false;
    }

    concurrent_log_test(logger::business::log, "data 2 = ", _work_2.m_msg);
    if ((_work_2.m_msg.m_i != 2000) &&
        (_work_2.m_msg.m_f != 3486784401.0)) {
      return false;
    }

    concurrent_log_test(logger::business::log, "data 3 = ", _work_3.m_msg);
