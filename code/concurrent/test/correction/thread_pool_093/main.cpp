...
struct thread_pool_093
{
  bool operator()()
  {
    ...
    concurrent_log_test(logger::business::log, "starting pool");
    _pool_1.run();
    ...
    concurrent_log_test(logger::business::log, "stopping pool");
    _pool_1.stop();

    concurrent_log_test(logger::business::log, "moving pool");
    thread_pool _pool_2(std::move(_pool_1));

    concurrent_log_test(logger::business::log, "sleeping for 3 s");
    std::this_thread::sleep_for(std::chrono::seconds(3));

    concurrent_log_test(logger::business::log, "starting new pool");
    _pool_2.run();
