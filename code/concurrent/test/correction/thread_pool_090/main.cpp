...
//typedef tenacitas::logger::business::log log;

typedef tenacitas::concurrent::tst::msg_a msg;
typedef tenacitas::concurrent::business::thread_pool_t<msg, logger::business::log>
thread_pool;
typedef tenacitas::concurrent::business::sleeping_loop_t<void, logger::business::log>
sleeping_loop;
...
struct thread_pool_090
{
  bool operator()()
  {
    ...
    sleeping_loop _loop(std::chrono::milliseconds(500),
      ...
      msg _msg(++_value);
      concurrent_log_test(
            logger::business::log, "adding msg ", _msg);
    ...
    concurrent_log_test(log, "sleeping for 10 secs");
    std::this_thread::sleep_for(std::chrono::seconds(10));
    concurrent_log_test(log, "waking up after 10 secs");

    concurrent_log_test(log, "stopping the pool");
    _pool.stop();

    concurrent_log_test(log, "sleeping for 5 secs");
    std::this_thread::sleep_for(std::chrono::seconds(5));
    concurrent_log_test(log, "waking up after 5 secs");

    concurrent_log_test(log, "runnig the pool");
    _pool.run();

    concurrent_log_test(log, "sleeping for 4 secs");
    std::this_thread::sleep_for(std::chrono::seconds(4));
    concurrent_log_test(log, "waking up after 4 secs");

    concurrent_log_test(
          log, "consumed = ", _work.m_msg.counter(), ", provided = ", _value);
    if (_work.m_msg.counter() != _value) {
      concurrent_log_error(
            log, "Data value consumed should be equal to provided");
      return false;
    }
