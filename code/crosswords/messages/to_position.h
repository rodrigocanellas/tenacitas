#ifndef TENACITAS_CROSSWORDS_MESSAGES_TO_POSITION_H
#define TENACITAS_CROSSWORDS_MESSAGES_TO_POSITION_H
...
struct to_position
{
                                  ...
                                  const to_position& p_to_position)
  {
    p_out << "to_positon msg: "
          << crosswords::entities::print_words(p_to_position.m_words.begin(),
                                   p_to_position.m_words.end());
    return p_out;
  }

  to_position() = default;
  inline explicit to_position(const crosswords::entities::words& p_words,
  const positions_occupied & p_positions_occupied)
    : m_words(p_words),
      m_positions_occupied(p_positions_occupied)
  {}

  inline to_position(crosswords::entities::words::const_iterator p_begin,
