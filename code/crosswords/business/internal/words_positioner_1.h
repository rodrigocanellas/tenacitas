#ifndef TENACITAS_CROSSWORDS_BUSINESS_WORDS_POSITIONER_1_H
#define TENACITAS_CROSSWORDS_BUSINESS_WORDS_POSITIONER_1_H
...
/// \brief words_positioner_1_t tries to position a set of words
///
...
struct words_positioner_1_t
{
  ...
  words_positioner_1_t(x p_x_limit, y p_y_limit)
    : m_x_limit(p_x_limit)
    , m_y_limit(p_y_limit)
  {}

  words_positioner_1_t() = default;
  words_positioner_1_t(const words_positioner_1_t&) = default;
  words_positioner_1_t(words_positioner_1_t&&) noexcept = default;
  words_positioner_1_t& operator=(const words_positioner_1_t&) = default;
  words_positioner_1_t& operator=(words_positioner_1_t&&) noexcept = default;
  ~words_positioner_1_t() = default;

bool operator()(words::iterator p_begin,
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (_ite->positioned()) {
          ...
          positioning _positioning = position(p_begin, p_end, _ite, p_positions_occupied);
          if (_positioning == positioning::ok) {
            ...
            //              m_y_limit));
            crosswords_log_debug(log, words(p_begin, p_end).print_words());
            ++_ite;
          } else {
...
private:
                       ...
                       positions_occupied & p_positions_occupied)
  {
    ...
    if (no_word_positioned(p_first_positioned, p_end)) {
      if (!position_first(p_to_position, p_positions_occupied)) {
    ...
    return position_from_last(p_first_positioned,
                              p_end,
                              p_to_position,
                                 ...
                                 positions_occupied & p_positions_occupied)
  {
    ...
    while (true) {
      ...
      if (_last_positioned == p_first_positioned) {
        bool _is_positioned =
          position_x(p_first_positioned, _last_positioned,
                     p_to_position, p_positions_occupied);
