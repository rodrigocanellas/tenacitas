...
#include <crosswords/business/positioner.h>
...
struct positioning_000
{
    ...
    explicit positioning_000(std::string&& p_file_name)
    {
        ...
        while (true) {
            ...
            m_positioner.add(std::move(_line),
                             "word #" + to_string(_counter++));
        }
    }
  ...
  private:
    typedef positioner_t<tenacitas::logger::business::log> positioner;
...
main(int argc, char** argv)
{
    ...
    try {
        if (argc != 4) {
            cout
              << "Syntax: " << argv[0] << " <file-name> <log-level> <action>"
              << endl
              << endl
