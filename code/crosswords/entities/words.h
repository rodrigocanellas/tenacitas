...
struct words {
    ...
    void add(word && p_word) {
        m_list.push_back(std::move(p_word));
    }
    ...
    std::string print_positioned(x p_x_limit, y p_y_limit) const {
        ...
        words::const_iterator _end = end();
        for (words::const_iterator _ite = begin(); _ite != _end; ++_ite) {
            if (_ite->positioned()) {
...
struct print_positioned_words {
    void
    operator()(coordinate::x p_x_limit, coordinate::y p_y_limit, const words& p_words)
    {
        using namespace std;
