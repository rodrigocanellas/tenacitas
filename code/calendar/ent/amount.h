...
struct amount
{
  ...
  inline amount operator+(amount p_amount)
  {
    return amount(m_value + p_amount.m_value);
  }
  ...
  inline amount operator*(amount p_amount) const
  {
    return amount(m_value * p_amount.m_value);
  }
  ...
  inline bool operator > (const amount & p_amount) const {
