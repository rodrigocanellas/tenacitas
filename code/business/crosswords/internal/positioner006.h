#ifndef TENACITAS_BUSINESS_CROSSWORDS_POSITIONER006_H
#define TENACITAS_BUSINESS_CROSSWORDS_POSITIONER006_H
...
#include <business/crosswords/internal/words_positioner_group.h>
    ...
    , m_words_positioner_group(p_x_limit, p_y_limit)
  {
    ...
    if (!m_dispatcher_created) {
      dispatcher_positioned::subscribe(
        [this](messages::crosswords::positioned<log>&& p_positioned) -> bool {
          return m_words_positioner_group(std::move(p_positioned));
        },
        std::chrono::milliseconds(1000));

      dispatcher_not_positioned::subscribe(
        [this](messages::crosswords::not_positioned&& p_not_positioned) -> bool {
