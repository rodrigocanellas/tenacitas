...
struct file_controller
{
  ...
  inline explicit file_controller(
    ...
    , m_deleter(
        ...
        m_base_name,
        m_closed_extension,
        std::chrono::seconds(business::calendar::min2sec(p_retention.count())))
    , m_sleeping_loop(
        std::chrono::milliseconds(
          business::calendar::min2mil(p_retention.count())),
        [this]() -> concurrent::result { return this->m_deleter(); },
...
private:
  ...
  ///
  typedef business::concurrent::sleeping_loop_t<void, logger::log>
    sleeping_loop;
