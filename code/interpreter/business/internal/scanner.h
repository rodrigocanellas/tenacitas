...
//    /// \brief
//    /// \param [in] p_tokens is the catalog of @p symbol objects that should
//    be
...
//    /// the text, which begin and end were defined using the @p set_text
//    method
//    /// \return a tenacitas::interpreter::symbol reference to the symbol read
//    /// from the text
//    /// \note If all the text has been read, the
...
////    if (rc) {
////        /// a tenacitas::interpreter::symbol was recognized, so we update
///the /        /// current position to be analysed in the text
...
////        text,
////        /// so that we try to recognize a @p symbol with another @p
///recognizer /        l_walker = m_walker; /    }
