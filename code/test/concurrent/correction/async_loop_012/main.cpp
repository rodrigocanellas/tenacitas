...
struct work1
{
  ...
  {
    data += p_data;
    concurrent_log_test(logger::log, "1 -> ", data);
...
struct work2
{
  concurrent::result operator()(uint32_t&& p_data)
  {
    using namespace tenacitas::business;
    data = p_data;
    concurrent_log_test(logger::log, "2 -> ", data);
    return concurrent::result::dont_stop;
  }
  uint32_t data = 0;
};
...
struct async_loop_012
{
  bool operator()()
  {
    ...
    try {
      ...
      concurrent_log_test(logger::log, "work 1 data = ", _work_1.data);
      concurrent_log_test(logger::log, "work 2 data = ", _work_2.data);
