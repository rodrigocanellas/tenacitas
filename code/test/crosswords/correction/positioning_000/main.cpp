...
#include <business/crosswords/positioner.h>
#include <entities/crosswords/board.h>
...
using namespace tenacitas::entities::crosswords;
using namespace tenacitas::business::crosswords;
using namespace tenacitas::business::logger;

struct positioning_000
{

  explicit positioning_000(std::string&& p_file_name)
  {
    using namespace std;
    ifstream _file(p_file_name);
    if (!_file.is_open()) {
      throw std::runtime_error("Could not open file '" + p_file_name + "'");
    }
    ...
    if (_line.substr(0, _result.size()) != "RESULT") {
      throw std::runtime_error("Expected RESULT key word, but parsed '" +
                               _line + " instead");
    }
    ...
    if (_pos == string::npos) {
      throw std::runtime_error("Could not find value of RESULT key word");
    }
    ...
    getline(_file, _line);
    if (!_file.good()) {
      throw std::runtime_error("Error reading number of columns");
    }
    coordinate::x _x_limit(atoi(_line.c_str()));

    getline(_file, _line);
    if (!_file.good()) {
      throw std::runtime_error("Error reading number of rows");
    }
    ...
    while (true) {
      ...
      if (!_file.good()) {
        throw std::runtime_error("Error reading word #" +
                                 to_string(_counter++));
      }
...
private:
  typedef positioner_t<tenacitas::business::logger::log> positioner;
...
{
  using namespace std;
  tenacitas::business::logger::configure_cerr_log();

  try {
    if (argc != 4) {
      cout << "Syntax: " << argv[0] << " <file-name> <log-level> <action>"
           << endl
           << endl
           << "Where <log-level> can be 'debug', 'info', 'warn' or 'error'"
           << endl
           ...
           << endl
           << endl
           << "The RESULT line indicates if 'all', 'some', or 'none' words "
