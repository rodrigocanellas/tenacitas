...
protected:
  ...
  using queue = src::container::circular_queue<logger, event, 20>;
...
struct handling_001 : public handling_test {
  ...
  bool operator()(const src::program::options &) {
    ...
    m_logger.tst(src::format::fmt("is stopped = ", _is_stopped));
    return !_is_stopped;
  }
};
...
struct handling_009 : public handling_test {
  ...
  bool operator()(const src::program::options &) {
    ...
    m_logger.tst(src::format::fmt("is stopped before? ", _is_stopped));

    _handling.stop();

    _is_stopped = _handling.is_stopped();
    m_logger.tst(src::format::fmt("is stopped after? ", _is_stopped));
