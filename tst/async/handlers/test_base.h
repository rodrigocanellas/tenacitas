...
template <event_id evt_id,
          size_t num_senders,
          size_t events_per_sender,
          size_t interval_per_sender,
          size_t num_handlers,
          size_t handlers_timeout,
          size_t timeout_at_each,
          size_t handler_sleep = handlers_timeout / 2>
struct test_t {
    ...
    typedef std::vector<size_t> values;
    ...
    bool operator()() {
        ...
        std::condition_variable _cond_all_sent;
        std::mutex _mutex_all_sent;
        ...
        for (size_t _i = 0; _i < num_handlers; ++_i) {
            _handler_list.push_back(make_sptr<handler>(
        ...
        INF("adding each event handler to the event handlers group");
        for (sptr<handler> &_handler : _handler_list) {
            _event_handlings.add_handler(
                [&](sptr<const bool> p_bool,
        ...
        for (size_t _i = 0; _i < num_senders; ++_i) {
        ...
        INF("************************** checking if all the ", m_total_events,
            " events were sent");
        {
            std::unique_lock<std::mutex> _lock(_mutex_all_sent);
            _cond_all_sent.wait(_lock, [&]() -> bool {
                if (_sent >= m_total_events) {
                    INF("************************** all ", m_total_events,
                        " events were sent");
                    return true;
                }
        ...
        INF("************************** waiting for all the handlers to handle "
            "the ",
            m_total_events, " events");
        {
            std::unique_lock<std::mutex> _lock(_mutex_all_handled);
            _cond_all_handled.wait(_lock, [&]() -> bool {
                if (_handled >= m_total_events) {
                    INF("all ", m_total_events,
                        " ************************** all ", m_total_events,
                        " events handled");
...

        {
            ...
            _file << static_cast<size_t>(evt_id) << ',' << num_handlers << ','
...
private:
    // event sent each time a work event is handled
    struct handled {
    ...
    struct handler {
        handler(size_t p_id,
                ...
                size_t p_num_max_timeouts,
                size_t p_cause_timeout_at_each = 0)
        ...
        //        inline size_t get_num() const { return m_num; }
    ...
    private:
        ...
        const size_t m_num_max_timeouts {0};
        const time m_sleep;
        const size_t m_cause_timeout_at_each {0};
        const time m_to_timeout;
        size_t m_num {0};
        size_t m_num_aux {0};
        event m_event;
        size_t m_timeout_counter {0};
    };
...
private:
    ...
    static constexpr size_t m_total_events {num_senders * events_per_sender};
    static constexpr size_t m_max_events_timeout {m_total_events /
                                                  num_handlers * 30 / 100};
};
