...
struct worker_000 {
  ...
  typedef concurrent::message_queue_t<log,
  ...
  bool operator()() {
    ...
    message_queue::operation _worker = [&_consumer](data &&p_data) -> void {
      _consumer(std::move(p_data));
    };

    on_timeout _on_timeout = [this]() -> void {
      WAR(m_log, "timeout!!");
    };

    _msg_queue.add(_worker, std::chrono::milliseconds(500), _on_timeout);
...
struct worker_001 {
  ...
  typedef concurrent::message_queue_t<log,
    concurrent::queue_type::CIRCULAR_FIXED_SIZE, msg> message_queue;
  typedef concurrent::on_timeout on_timeout;
  ...
  bool operator()() {
    ...
    _msg_queue.add(
        ...
        std::chrono::milliseconds(2000), _on_timeout);
...
private:
  struct producer {
    producer(message_queue *p_msg_queue, msg *p_msg) : m_msg_queue(p_msg_queue), m_msg(p_msg) {}
...
struct worker_002 {
  ...
  typedef concurrent::sleeping_loop_t<log, void> sleeping_loop;
  typedef concurrent::message_queue_t<log,  concurrent::queue_type::CIRCULAR_FIXED_SIZE, msg> message_queue;
  typedef concurrent::on_timeout on_timeout;
...
//    producer _producer(&_msg_queue, &_msg);
    ...
    sleeping_loop _loop(
        300ms, 500ms, producer{&_msg_queue, &_msg},
         _on_timeout);

    DEB(m_log, "adding consumer to the worker");
    _msg_queue.add( [&_consumer](msg &&p_msg) { return _consumer(std::move(p_msg)); },
            100ms, _on_timeout);
  ...
  struct producer {
    producer(message_queue *p_msg_queue, msg *p_data) : m_msg_queue(p_msg_queue), m_msg(p_data) {}
...
struct worker_003 {
  ...
  bool operator()() {
    ...
    _msg_queue.add(
        ...
        std::chrono::milliseconds(1000), _on_timeout);
    ...
    msg::number _amount_added = static_cast<msg::number>(_msg_queue.amount_added());
