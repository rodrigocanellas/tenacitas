...
struct cout_log_how_to {
  ...
  static std::string name() { return "cout_log_how_to"; }
};
...
struct cout_log_single {
  ...
  bool operator()() {
    try {
      typedef concurrent::_bus::sleeping_loop_t<void, logger::cout::log>
          sleeping_loop;
...
int main(int argc, char **argv) {
  ...
  _test.run<cout_log_creation>();
  _test.run<cout_log_how_to>();
  _test.run<cout_log_single>();
  _test.run<cout_log_multi>();
}
